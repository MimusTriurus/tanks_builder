...
```bash
curl -X POST http://localhost:8427/api/tools/node-duplicate \
...
> Or pipe via stdin:
> ```bash
> curl -X POST http://localhost:8427/api/tools/node-duplicate -H "Content-Type: application/json" -d @- <<'EOF'
> {"param": "value"}
> EOF
> ```
