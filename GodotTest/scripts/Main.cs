...
public sealed partial class Main : Node2D
{
	...
	/// of it. Both in the shooter's global space, which is the harness's canvas
					   ...
					   int serial)
	{
		...
		(Vector2 launched, Vector2 along) =
			BoreOf(shooter, shooter.Sprite.TurretFacing);
		...
		Vector2 bore = victim.Sprite.ToLocal(launched + along) - eye;
	...
	private void Sight(Vehicle shooter, Vehicle? victim, double heading)
	{
		...
		if (victim is not null && at == victim.Cell
			&& Aim(shooter, victim, Mod(heading + 180.0, 360.0),
				   victim.HitCount + 1) is Aimed shot)
		{
			Onto(shooter, victim, shot);
			return;
		}
	...
	/// </summary>
	private void Onto(Vehicle shooter, Vehicle victim, Aimed shot)
	{
		Vector2 hole = victim.Sprite.ToGlobal(shot.Impact);
		if (!Staged)
		{
			_aimRay!.Aim(shot.Muzzle, hole, AimRay.Tip.Hole);
			return;
		}
		if (_stage!.Screen(shooter, shot.Muzzle) is not Vector2 gun
