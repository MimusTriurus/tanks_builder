...
CONFIG = {
    ...
    "hull_root": None,
    "turret_root": None,
    # **The gun is not armour, and this list is where that is said.** Names -
    # and everything under them - to leave out of the hull group.
    #
    # It used to need saying nowhere, because the tube hung off the turret root
    # and no walk of the hull root could reach it. On a casemate it hangs off
    # the *hull* root, because nothing on such a vehicle turns against the hull,
    # and three separate measurements quietly took the tube for part of the
    # tank: the plate rays, the scar's seat probe and the burst's height.
    #
    # Measured on TD_StuG4, whose tube reaches 0.39 further forward than its
    # glacis: the front plate came out 45.1 deg instead of 41.6 with an extent
    # half again as tall, and the front scar's seat climbed out past the muzzle -
    # `scar_off_armour` 1.0, the whole mark off the tank and drawn at every
    # heading. A rule that held for free by hierarchy, and stopped.
    "exclude": (),
...
def _targets(hull, exclude=()):
    """The meshes a plate ray may land on: the hull group, less the gun.

    See CONFIG["exclude"] for why the gun has to be named at all.
    """
    skip = set()
    for name in exclude or ():
        ob = bpy.context.scene.objects.get(name)
        if ob is not None:
            skip.update(o.name for o in [ob] + list(ob.children_recursive))
    return [o for o in [hull] + list(hull.children_recursive)
