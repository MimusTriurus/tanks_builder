...
CONFIG = {
    ...
    # **Whether the mount can fall into a ring, which is whether it has one.**
    # False is a casemate: the box is welded to the hull, so it cannot drop, tip
    # or be thrown clear, and the only thing on the vehicle whose shape changes
    # when it dies is the gun falling to its stop. The cant, the pitch, the ring
    # and the void under the mount all go with it - see `barrel_layer`.
    #
    # Not derived from "is there a ring stamp": a stale stamp left on a hull by
    # an earlier run would answer yes, and the answer would be a picture of a
    # welded box tipping over. It is handed down from `tank_parts.turreted`,
    # which reads the vehicle's own shape.
    "cant": True,
...
def _trunnion(cfg):
    ...
    that does not happen to lie along an axis of the scene.

    **Delegated to `barrel_recoil.trunnion`**, which is where the fit now lives.
    It was here first and elevation wanted the same point; two fits of one pivot
    would have agreed wherever anybody compared them and parted company on the
    one tank nobody did - the dropped gun of a wreck and a laid gun would lay
    about different points and no output number would say so.
    """
    import mathutils
    br = importlib.import_module("barrel_recoil")
    sub = {"barrel": cfg["barrel"], "turret": cfg["turret"], "root": cfg["root"]}
    pivot, axis, _rep = br.trunnion(sub)
    return (mathutils.Vector(pivot), mathutils.Vector(axis), br.reach(sub))
...
def measure(cfg=None):
    ...
    import mathutils
    if not cfg.get("cant", True):
        # No ring, so no cant, no pitch and no void to open under the mount.
        # What is left is the droop, and it is reported in the same shape so a
        # reader of the atlas metadata does not need a second case.
        out = {"cant_deg": 0.0,
               "cant_from": "welded mount: no ring to fall into",
               "pitch_deg": 0.0,
...
def _posed(cfg):
    """The children this pose moves: the mount and the gun, or the gun alone.

    The gun alone is the casemate - see `CONFIG["cant"]`. The mount is left out
    of the *pose*, not out of the picture: it stays where it is, in the hull
    layer, which is why a dead casemate keeps its box.
    """
    try:
    ...
    if not cfg.get("cant", True):
        if bar is None:
            raise RuntimeError(
                "a welded mount with no gun has no wreck pose at all - nothing "
                "on this vehicle changes shape when it dies")
        return [bar], bar
    tur = _mesh(cfg["turret"])
    if bar is None:
        return [tur], None
...
def pose_turret(cfg=None, strict=False):
    ...
    import mathutils
    cfg = dict(CONFIG, **(cfg or {}))
    canting = bool(cfg.get("cant", True))
    ring = _ring(cfg) if canting else None
    posed, bar = _posed(cfg)
    tur = posed[0] if canting else None
    piv = axis = None
    if bar is not None:
    ...
    # A welded mount stops here: the tube has dropped and nothing else on the
    # vehicle moved. This is the casemate, and it is the whole of its pose.
    if not canting:
        bpy.context.view_layer.update()
...
def barrel_layer(cfg=None, siblings=None):
    """The casemate's wreck: the gun dropped to its stop, and nothing else.

    The twin of `turret_layer` for a vehicle whose mount is welded on. There the
    tube is *inside* the mount layer, because the mount fell and took the gun
    with it; here the mount did not move, so it stays in the hull layer it was
    rendered in and the tube is the only thing with a pose of its own.

    `siblings` is the rest of what the layer root draws, for the reason
    `barrel_recoil.layers` gives: this layer's root is the hull, so without them
    it would draw the whole hull again on the gun's own heading.

    Indexed by the gun's heading exactly as the live barrel layer is - which
    costs nothing on a casemate, where that heading is the hull's, and keeps the
    two layers the same shape so the harness can swap one for the other.

    `fit: False` for `turret_layer`'s reason: a drooped tube reaches lower than a
    laid one, and letting it size the frame would shrink the live tank in every
    other layer.
    """
    cfg = dict(CONFIG, **(cfg or {}))
    layer = {"name": "wreck_barrel", "target": cfg["root"],
             "fit": False, "phases": 1, "phase_hook": _TurretHook(cfg),
             "meta_extra": {"wreck": measure(cfg)}}
    if siblings:
        layer["exclude"] = list(siblings)
        layer["holdout"] = list(siblings)
    return layer
