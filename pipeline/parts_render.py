...
CONFIG = {
    ...
    # The gun tube, and the welded box a casemate has instead of a turret. Both
    # are resolved through `tank_parts`, so the bare name serves either naming
    # convention; the casemate is absent on a turreted scene and unused there.
    "barrel": "Barrel",
    "casemate_mesh": "Casemate",
    ...
    # laying the gun in elevation: None keeps it level and the layer exactly as
    # it shipped, True takes `barrel_recoil.ladder()` - the angles the board can
    # ask for - and a list is an explicit table in degrees. It multiplies the
    # barrel layer by its length, because the stroke runs along the bore and the
    # bore is a different direction at every angle.
    "elev": None,
...
def hull_axis(cfg=None):
    """What the carousel turns about when the vehicle has no ring.

    The horizontal centre of **what stands on the tile** - the hull and the two
    belts, with the gun left out. Deliberately the same set `ground_tile` fits,
    so the pivot and the tile's centre are one point rather than two that happen
    to agree: `anchor_px` means nothing unless every layer turns about one axis.

    The gun is left out for the reason it is left out of the tile fit - what has
    to be inside the cell is the part that drives over it - and here it matters
    more than there, because on a casemate the tube hangs off the hull root. A
    barrel reaching half a hull-length forward would drag the pivot forward with
    it and the tank would orbit its own gun.

    **Why not a fitted ring.** `turret_axis` will happily fit one to a box, and
    the numbers it returns are the reason this exists: `ring_roundness` 0.71 on
    both casemates in this project, a fitted radius of 0.379 on a box that
    reaches 0.31, and on the destroyer an axis sitting 0.167 - a sixth of the
    hull's own length - behind the vehicle's centre. Spinning a carousel about
    that point walks the whole tank round a circle of that radius, which is the
    one failure no output number reports.

    **Why not the empty**, for the reason `ring_axis` gives at length: a
    `*.World` empty looks like a declared pivot and is one only by accident.
    """
    import bpy, importlib
    ...
    gun = tp.mesh(cfg["barrel"], required=False)
    skip = {o.name for o in hb.hierarchy(gun)} if gun is not None else set()
    pts = []
    for name in [cfg["hull"]] + list(cfg["tracks"]):
        for ob in hb.hierarchy(bpy.data.objects[name]):
            if ob.type != "MESH" or not ob.data.vertices or ob.name in skip:
                continue
            pts.extend(ob.matrix_world @ Vector(c) for c in ob.bound_box)
    if not pts:
        raise RuntimeError("nothing to centre the carousel on: %s hold no mesh"
                           % ([cfg["hull"]] + list(cfg["tracks"])))
    xs = [p.x for p in pts]
    ys = [p.y for p in pts]
    return {
        "axis": [round((min(xs) + max(xs)) / 2.0, 5),
                 round((min(ys) + max(ys)) / 2.0, 5)],
        "from": "footprint centre of the hull and belts, gun excluded",
        "gun_excluded": sorted(skip),
        # no ring, so none of the ring's numbers - said out loud rather than
        # left as absent keys a reader has to guess the meaning of
        "roundness": None,
        "seam_z": None,
        "axis_warnings": [],
        "rotatable": {"free": True,
                      "why": "nothing turns against the hull on a casemate"},
    }


def _siblings(root_name, gun_name):
    """Everything the layer root draws except the gun and whatever hangs off it.

    What the tube has to be excluded from, and what has to be excluded from the
    tube's own layer - see `barrel_recoil.layers`. On a turreted scene this is
    the mount alone and the caller does not need it; on a casemate the root is
    the hull, so it is the hull plates, the engine and the box.
    """
    import bpy, importlib
    hb = importlib.import_module("hex_base")
    gun = bpy.data.objects[gun_name]
    skip = {o.name for o in hb.hierarchy(gun)}
    return sorted(o.name for o in hb.hierarchy(bpy.data.objects[root_name])
...
class Body:
    ...
    def __init__(self, cfg=None):
        ...
        tp = importlib.import_module("tank_parts")
        # Read from the scene, never configured, for `tank_parts.layout`'s
        # reason: a flag saying "no turret" on a scene that has one would be two
        # answers to one question and the stale one wins silently.
        self.turreted = tp.turreted()
        # The mount the tube slides into and droops out of - a turret, or a
        # casemate's welded box. Not the same question as the ring: a casemate
        # has a mount and no ring, and every measurement here about the box
        # (travel ceiling, trunnion, holdout) is about the mount.
        self.mount = (cfg["turret_mesh"] if self.turreted
                      else tp.mesh(cfg["casemate_mesh"]).name)
        # And the root its layer is drawn from. On a casemate the box and the
        # gun both hang off the hull, because nothing on the vehicle turns
        # against the hull - so there is no second root to hold them.
        self.mount_root = cfg["turret"] if self.turreted else cfg["hull"]
        if self.turreted:
            self.spin = ring_axis(cfg)
            self.ring = ring_plane(cfg, self.spin["axis"])
        else:
            self.spin = hull_axis(cfg)
            # No ring means no deck to cut the mount's basket out of, and none
            # is wanted: the box is *in* the hull layer, so Blender's own depth
            # sorts it against the deck the way a 3D render does. The cut box
            # exists only because a separately-rendered mount has no depth to
            # sort by - see the module docstring.
            self.ring = {"cut_z": None, "deck_z": None,
                         "note": "no ring: the mount is in the hull layer, so "
                                 "there is nothing to cut it out of"}
        ...
        hull_layer = {"name": "hull", "target": cfg["hull"]}
        turret_layer = ({"name": "turret", "target": cfg["turret"]}
                        if self.turreted else None)
        ...
        # the gun tube leaves the mount's layer for one of its own, so that it
        # can recoil. Both layers keep the mount's root: the tube is a child of
        # it and a layer root may not be a child of another layer root. On a
        # casemate that root is the hull, so it is the *hull* layer the tube is
        # shorn from, and the hull's other meshes that the tube's layer holds out.
        self.barrel = None
        ...
        if int(cfg["recoil_phases"] or 0) > 1:
            ...
            try:
                if not self.turreted:
                    self.siblings = _siblings(
                        self.mount_root,
                        importlib.import_module("tank_parts")
                        .mesh(cfg["barrel"]).name)
                shorn, self.barrel = br.layers(
                    dict(barrel=cfg["barrel"], turret=self.mount,
                         root=self.mount_root,
                         phases=int(cfg["recoil_phases"])),
                    elevations=cfg["elev"], siblings=self.siblings)
                ...
                # whichever layer the tube came out of is the one that excludes
                # it, and `shorn` is that layer's half of the pair
                (turret_layer if self.turreted
                 else hull_layer)["exclude"] = list(shorn["exclude"])

        self.layers = [hull_layer] + ([turret_layer] if turret_layer else []) + [
        ...
        if self.barrel is not None:
            # right after the mount it came out of, so the composite order in
            # the sheets is the order the game draws
            self.layers.insert(2 if self.turreted else 1, self.barrel)
        self.shadow = None
        # **What does not belong to the ground**, for the tile that is fitted to
        # it and the shadow that is cast onto it. One set, because both answer
        # the same question - what drives over the cell - and two answers to it
        # would drift.
        #
        # The stale belts are here for their own reason (each track root holds
        # the delivered belt and the rebuilt copy). **The gun joins them on a
        # casemate, and it has to:** the tube hangs off the hull root there, so
        # the tile would be sized to clear its overhang and the shadow would be
        # cast by it. Both rules were already written in this file - what must
        # stay inside the cell is the part that drives over it, and what casts
        # is the hull and the belts and nothing else - and both used to hold for
        # free, because the gun was under a root neither of them walked.
        #
        # Measured when it did not: `shadow_interior_min` 158 -> 70 on the first
        # casemate render, with the barrel casting.
        self._off_ground = {n for s in drop.values() for n in s}
        if not self.turreted:
            gun = importlib.import_module("tank_parts").mesh(
                cfg["barrel"], required=False)
            if gun is not None:
                hb = importlib.import_module("hex_base")
                self._off_ground |= {o.name for o in hb.hierarchy(gun)
                                     if o.type == "MESH"}
        if cfg["hex"] is not None:
            tile, self.hex = ground_tile(cfg, self.spin["axis"],
                                         self._off_ground)
    ...
    def pose_wreck(self):
        """A callable that puts the mount in the wreck's pose, or None.

        The mount, not the turret: on a casemate the pose is the gun dropping to
        its stop and the box does not move at all - see wreck_pose's `cant`.
    ...
    def _add_wreck(self, cfg):
        ...
        import importlib
        wp = importlib.import_module("wreck_pose")
        base = dict({"root": self.mount_root, "turret": self.mount,
                     "barrel": cfg["barrel"], "cant": self.turreted,
        ...
        try:
            ...
            wp.capture(base)
            # A welded mount does not fall, so there is no `wreck_turret` to
            # render: the box stays in the hull layer the live tank drew it in,
            # and the drooped tube is the only pose. See wreck_pose.barrel_layer.
            spec = [wp.turret_layer(base) if self.turreted
                    else wp.barrel_layer(base, self.siblings)]
        except (KeyError, RuntimeError) as exc:
...
def check(cfg=None, headings=(2, 3, 4)):
    ...
    # Absent on a casemate, whose box is in the hull layer. This sheet is about
    # the belts, so it draws the tank around them and does not care which shape
    # the mount is - but it has to *load* only what is there, or a debug tool
    # that raises is a debug tool nobody runs.
    turr = (layer("turret")[0].reshape(-1, tile, tile, 4)
            if os.path.exists(os.path.join(d, "turret_atlas.json")) else None)
