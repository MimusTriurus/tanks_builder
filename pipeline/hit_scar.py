...
def _armour(cfg):
    """The meshes the decal has to sit clear of - and the gun is not one.

    See CONFIG["exclude"]: a mark seated to clear the muzzle is a mark in front
    of the tank, which is what this list was added to stop.
    """
    tp = _mod("tank_parts")
    ...
    skip = set()
    for name in cfg.get("exclude") or ():
        ob = scene.objects.get(name)
        if ob is not None:
            skip.update(o.name for o in tp.hierarchy(ob))
