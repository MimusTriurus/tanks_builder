...
def block(folder, report=None):
    """The ports block for one sprite folder, or None if there is no port.

    `report` is the run report already in hand - see `stamp_bore.bore_axis` on
    why `tank_pipeline.run` stamps before it writes the report rather than
    after.
    """
    if report is None:
...
def stamp(folder, dry_run=False, report=None):
    """Write the block into every burning layer of one folder."""
    made = block(folder, report)
    if made is None:
