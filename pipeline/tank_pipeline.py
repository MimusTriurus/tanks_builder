...
split is a minute of hand work per tank. See docs/MUZZLE_FLASH.md for the gun.
...
REPO = r"D:\Projects\AgentCoding\BlenderMCP\pipeline"
...
CONFIG = {
    ...
    "recoil_phases": 5,
    # laying the gun in elevation, on the same layer and as its outer axis:
    # None keeps the gun level and the set exactly as they shipped, True takes
    # `barrel_recoil.ladder()`, a list is a table in degrees. Forwarded to
    # `parts_render` - which is where the knob above lived for a while with
    # nothing carrying it here, and a knob that does nothing is
    # indistinguishable from a knob that did not help.
    "elev": None,
    ...
    # of on its axis - see docs/MUZZLE_FLASH.md.
...
# `wreck_barrel` is the casemate's half of that argument: the box is welded on
# and cannot fall, so the only thing on a casemate that changes shape when it
# dies is the gun dropping to its stop. On a turreted tank the drooped tube is
# *inside* `wreck_turret` and this layer does not exist; on a casemate there is
# no `wreck_turret` and this is the whole of the pose. `_atlases` skips whatever
# has no file, so one order serves both.
WRECK_LAYERS = ("wreck_turret", "wreck_barrel",
...
def parts(cfg=None):
    ...
    return {"layout": kind,
            # A ring or no ring, and it is `tank_parts` that decides, off the
            # presence of the turret layer *root*. Every stage below asks this
            # one flag rather than inventing its own test - the places that each
            # assumed a turret were each a chance for one of them not to notice.
            # See tank_parts.turreted.
            "turret": named["turret"],
...
def _names(cfg, have):
    ...
    return {
        ...
        "hull_hint": cfg["hull"],
        "turret_hint": cfg["turret"] if named["turret"] else None,
        # Where `muzzle_point` left the bore stamps. It writes them onto the
        # mount *and* the tube, so on a casemate the tube is the only holder -
        # the same stamp, not a stand-in for it, which is why the readers below
        # are handed this instead of each testing for a turret of its own.
        "stamped_gun": (named["turret_mesh"] if named["turret"]
                        else named["barrel_mesh"]),
        # **What is not part of the tank for a measurement's purposes: the gun.**
        # The plate rays, the scar's seat probe and the burst's height all walk
        # the hull root, and on a casemate the tube hangs off it - see the
        # `exclude` note in hit_point.CONFIG for what that measured wrong.
        #
        # Only on the parts layout. On a single-mesh scene the hull layer *is*
        # the whole tank and the gun has always been in these groups; taking it
        # out would change sets that are not being re-rendered, which is a
        # different decision from fixing this one.
        "not_armour": ([named["barrel_mesh"]]
                       if named["layout"] == "parts" and named["barrel_mesh"]
                       else []),
    }
...
def prepare(cfg=None):
    ...
    if not have["flash"] and not have["exhaust"]:
        print("[tank] no %r and nothing starting with %r: this scene gets the "
              ...
              "onto %s for the gun or %s for the exhaust. See docs/MUZZLE_FLASH.md."
              % (cfg["barrel"], cfg["exhaust"],
                 cfg["turret"] if have["turret"] else cfg["hull"], cfg["hull"]))
    ...
    held = [] if not have["turret"] else [
        n for n in (hull, turret) if "ring_axis" in scene.objects[n].keys()]
    if not have["turret"]:
        # No ring, so nothing to fit and nothing to warn about. Not an economy:
        # fitting a circle to a casemate produced `ring_roundness` 0.70 and a
        # radius (0.379) larger than the box it was fitted to, and then a
        # permanent `problems` line - "the turret still overlaps the hull 0.055
        # above the seam, so it will clip through it as it turns" - about a
        # motion this vehicle does not have. A warning that can never be cleared
        # is noise in the one channel that has to be loud.
        #
        # What the carousel spins about instead is the hull's own footprint
        # centre, through the route `sprite_atlas` already has for it:
        # `spin_pivot_from`. See parts_render.Body.
        out["axis"] = {"from": "no ring: this vehicle is a casemate",
                       "ring_axis": None}
    elif held:
    ...
    out["hits"] = _load(cfg, "hit_point").set_hits(
        {"hull": hull, "turret": turret, "hull_root": name["hull_root"],
         ...
         "exclude": name["not_armour"],
         "front_dir": cfg["front_dir"], "azimuth": cfg["azimuth"],
         "elevation": cfg["elevation"], "camera_elevation": cfg["elevation"],
         "stamp_on": [hull]})
    if have["flash"]:
        # On a casemate the tube hangs off the hull, so that is the root it has
        # to be inside, and there is no ring bearing to check the bore against -
        # `muzzle_point` falls back to the hull's own front, which is the axis
        # that means anything here.
        out["muzzle"] = _load(cfg, "muzzle_point").set_muzzle(
            {"turret": turret, "barrel": name["barrel_mesh"],
             "turret_root": name["turret_root"] or name["hull_root"],
             "stamp_on": ([turret, name["barrel_mesh"]] if have["turret"]
                          else [name["barrel_mesh"]])})
    if have["exhaust"]:
    ...
    if have["flash"]:
        ...
        gun = name["stamped_gun"]
        flash_mod.build({"turret": gun, "name": cfg["flash"]})
        flash_mod.build_smoke({"turret": gun, "name": cfg["smoke"]})
    if have["exhaust"]:
    ...
    scar_seat = {"hull": hull, "hull_root": name["hull_root"],
                 "exclude": name["not_armour"], "name": cfg["scar"]}
...
def render(cfg=None):
    ...
    blockers = [cfg["hull"], cfg["turret"]] if have["turret"] else [cfg["hull"]]
    ...
    if have["flash"]:
        ...
        layers.extend([
            {"name": "flash", "target": cfg["flash"],
             ...
             "phases": cfg["phases"],
             "phase_hook": flash_mod.phase_hook({"turret": named["stamped_gun"],
                                                 "name": cfg["flash"]}),
            ...
            {"name": "smoke", "target": cfg["smoke"], "fit": False,
             ...
             "phase_hook": flash_mod.smoke_phase_hook(
                 {"turret": named["stamped_gun"], "name": cfg["smoke"]}),
             "holdout": blockers},
        ])
    ...
    scar_cfg = {"hull": hull, "hull_root": named["hull_root"],
                "exclude": named["not_armour"], "name": cfg["scar"]}
    ...
    # Nothing to order against on a casemate, and nothing that needs it: the box
    # is *in* the hull layer, so every hull-indexed effect holds the whole
    # silhouette out exactly, at every heading, and the harness draws a
    # held-out layer under the mount - which is the same decision the flag
    # would have carried. See AtlasSet.OverTurret: the flag and the holdout are
    # two halves of one answer and they arrive together or not at all.
    order = _load(cfg, "draw_order") if have["turret"] else None
    ...
    angles = atlas.frame_angles({"steps": cfg["steps"], "start_angle": 0.0,
                                 "direction": "ccw"})
    for layer in layers:
        if order is None or layer["name"] not in ORDERED_LAYERS:
            continue
...
def _body(layers, heading, phase=0):
    ...
    # A casemate has no turret layer at all - its box is in the hull layer above,
    # and the sheets need no case of their own for that. See tank_parts.
    if "turret" in layers:
...
def check(cfg=None):
    ...
    # Which layer *is* the wreck's pose, and there are two shapes of answer.
    # A turret fell into its ring and took the gun with it, so one layer carries
    # both. A casemate's box is welded on and did not move - it is in the hull
    # layer, drawn on a dead tank exactly as on a live one - so the drooped tube
    # is the whole pose. See wreck_pose.barrel_layer.
    wreck_mount = next((n for n in ("wreck_turret", "wreck_barrel")
                        if n in layers), None)
    if wreck_mount is not None:
        def draw_wreck(buf, place, heading, dead):
            ...
            gun = ("wreck_barrel" if dead and "wreck_barrel" in layers
                   else ("barrel" if "barrel" in layers else None))
            if gun is not None:
                buf = place(buf, gun, heading, _over)
            return buf
        ...
        seated = "turret" if wreck_mount == "wreck_turret" else "barrel"
        live = _tile_of(layers, seated, 0)[:, :, 3]
        dead = _tile_of(layers, wreck_mount, 0)[:, :, 3]
        moved = int(np.count_nonzero((live > 0.5) != (dead > 0.5)))
        report["%s_moved_px" % wreck_mount] = moved
        if wreck_mount == "wreck_turret" and moved < 200:
            report["problems"].append(
        ...
        elif wreck_mount == "wreck_barrel":
            # **A fraction of the tube rather than a pixel count**, because a
            # tube is a few dozen pixels of a tank and an absolute floor written
            # for a canted turret would pass on any droop at all here. What has
            # to be true is that a real part of the gun ended up somewhere else:
            # the drop is `barrel_length * sin(droop_deg)` at the muzzle and
            # sweeps the tube's own area on the way.
            area = int(np.count_nonzero(live > 0.5))
            share = moved / float(area) if area else 0.0
            report["wreck_barrel_moved_share"] = round(share, 3)
            if share < 0.25:
                report["problems"].append(
                    "the wreck's gun differs from the live one by %d px of a "
                    "tube %d px across (%.0f%%) - the droop did not take, or it "
                    "is too small to read. The lever is "
                    "wreck_pose.CONFIG['droop_deg'], and on a welded mount this "
                    "is the *whole* pose: nothing else on the vehicle moves"
                    % (moved, area, 100.0 * share))
        ...
        best, per = 0, []
        # A casemate opens no void and is not expected to: the box is welded on,
        # never lifts, and the hole the generator leaves under a mount stays
        # covered by it in the hull layer. That is the whole of the decision
        # taken here - a dead casemate keeps its box - so the test that asks
        # whether a burnt-out compartment shows has nothing to ask.
        for h in (range(layers["hull"][1]["count"])
                  if wreck_mount == "wreck_turret" else ()):
        ...
        if wreck_mount == "wreck_turret" and best < 60:
            report["problems"].append(
    ...
    if "barrel" in layers:
        ...
        # The layer carries two axes when the gun lays: `phase = elev*ntr +
        # travel`. Every check below is about the *stroke*, so it has to fold
        # elevation away first - measured flat, phase 5 is the level gun's
        # neighbour and is actually the first rung of the ladder, so the recoil
        # reads as "reaches further than rest" and "renders the same pixels".
        # It said both on the first laid set, about frames that were right.
        elev_tab = (bm.get("elev") or {}).get("table_deg") or [0.0]
        ...
        report["recoil_composite"] = _sheet(
            cfg, layers, "_check_recoil", headings, list(range(ntr)),
            draw_recoil, _ground_colour(layers), _body_used())

        # The ladder gets a sheet of its own, and it has to come out of the run
        # rather than off someone's scratch script: a laid gun is the one pose
        # nothing else on any sheet shows, and a check nobody renders is a check
        # nobody ran. Angle across, heading down, at rest travel.
        if len(elev_tab) > 1:
            rungs = sorted(range(len(elev_tab)), key=lambda e: elev_tab[e])

            def draw_elev(buf, place, heading, rung):
                for layer, index in (_ground(layers, heading)
                                     + _body(layers, heading)):
                    if layer == "barrel":
                        continue
                    buf = place(buf, layer, index, _over)
                # a *frame* index, not a phase: phase * count + heading, the
                # same arithmetic as the recoil sheet above. Handing `place` a
                # phase drew a different heading's tube in every cell, and it
                # looked like the gun swinging in azimuth rather than laying.
                return place(buf, "barrel",
                             (rungs[rung] * ntr) * bm["count"] + heading, _over)

            report["elev_composite"] = _sheet(
                cfg, layers, "_check_elev", headings, list(range(len(rungs))),
                draw_elev, _ground_colour(layers), _body_used())
        ...
        for p in range(nph):
            ...
            reach.append(row)
        # against the *block's own* rest, so the stroke is a stroke at every
        # elevation and not a stroke plus however far the ladder moved
        stroke = [[None if (reach[(p // ntr) * ntr][h] is None
                            or reach[p][h] is None)
                   else round(reach[(p // ntr) * ntr][h] - reach[p][h], 2)
        ...
        col = [stroke[p][3 % bm["count"]] for p in range(ntr)]
        # **Asked across headings, not per cell.** A transposed or wrong-signed
        # travel table is wrong at every heading and by a lot; what one cell
        # catches instead is the ruler. `reach` is the farthest drawn pixel from
        # the anchor, and on the heading where the gun points at the camera the
        # tube is a stub of a couple of hundred pixels, so the argmax hops
        # between the muzzle rim and the mantlet and reads a pixel of overshoot.
        # Laying the gun made that visible: 13 cells over 1320, all on heading 0,
        # all on elevated rungs, worst 1.37px - and every one of them a pose that
        # is right. So the claim is "more than one heading shows it".
        over = sorted({h for row in stroke for h, s in enumerate(row)
                       if s is not None and s < -0.75})
        report["recoil_overshoot_headings"] = over
        if len(over) > 1:
            report["problems"].append(
                "the tube reaches further than rest at headings %s - that is "
                "not a recoil, and the phase table is the place to look" % over)
        elif ntr > 2 and not all(col[i] >= col[i + 1] - 0.35
                                 for i in range(1, ntr - 1)):
            report["problems"].append(
                "the stroke does not come home monotonically: %s" % col)
        flat = [p for p in range(2, ntr)
        ...
        # The ladder's own check, and it is the same claim one axis over: a rung
        # that renders its neighbour's pixels is a render done twice, and a
        # ladder that does not climb monotonically is a table in the wrong
        # order. Measured on the muzzle at rest travel, along the drawn tube's
        # own reach from the anchor - the quantity the stroke already uses.
        if len(elev_tab) > 1:
            rungs = sorted(range(len(elev_tab)), key=lambda e: elev_tab[e])
            h0 = 6 % bm["count"]        # broadside: the tube is least foreshortened
            # Asked as a share of the silhouette rather than as a distance,
            # because the question is literally "is this rung a second copy of
            # its neighbour". A far point is the cheaper ruler and the wrong one:
            # it is an integer, and on a tube with a muzzle brake it hops between
            # features - it read 11px where the muzzle moved 5. What changed
            # cannot hop.
            masks = [_tile_of(layers, "barrel", e * ntr + h0)[:, :, 3] > 0.5
                     for e in rungs]
            moved = [round(float((masks[i] ^ masks[i - 1]).sum())
                           / max(float(masks[i].sum()), 1.0), 3)
                     for i in range(1, len(masks))]
            report["elev_table_deg"] = [elev_tab[e] for e in rungs]
            report["elev_rung_moved"] = moved
            # what the mount allows, measured by `barrel_recoil.trunnion` at
            # render time and carried in the layer's own metadata
            mount = (bm.get("elev") or {})
            report["elev_clearance_in_radii"] = mount.get("clearance_in_radii")
            report["elev_offset_at_cap"] = mount.get("offset_at_cap")
            report["problems"].extend(mount.get("warnings") or [])
            same = [i for i, m in enumerate(moved) if m < 0.08]
            if same:
                report["problems"].append(
                    "elevation rungs %s redraw their neighbour's tube almost "
                    "exactly (%s of the silhouette changes) - that is one render "
                    "done twice, so drop them from `barrel_recoil.ladder`"
                    % ([report["elev_table_deg"][i + 1] for i in same],
                       [moved[i] for i in same]))
...
def _pil_python(cache={}):
    """An interpreter that can read a PNG, which Blender's cannot.

    **`stamp_bore` and `stamp_ports` import PIL, and Blender ships no PIL.** So
    the seam `run()` calls them through cannot import them at all - which was
    not noticed when it was written, because it was exercised under the system
    Python with `bpy` stubbed out. Under a stub every import resolves; in the
    only place that matters neither module loads. The mechanism was a report of
    a mechanism.

    `sys.executable` first, so outside Blender this is a no-op and nothing forks.
    Then the launcher `py`, which is what resolves here: the bare `python` on
    PATH is the Store stub and raises "cannot find the path" when Blender is its
    parent. Each candidate is asked to import PIL rather than assumed to have
    it, because an interpreter that cannot is exactly as useless as none.

    None means no route, and the caller has to be loud about that rather than
    quietly skip the stamps - see `_stamp`.
    """
    import shutil
    import subprocess
    if "exe" in cache:
        return cache["exe"]
    cache["exe"] = None
    for cand in (sys.executable, shutil.which("py"), shutil.which("python")):
        if not cand:
            continue
        try:
            done = subprocess.run([cand, "-c", "import PIL, numpy"],
...
def _stamp_apart(cfg, module, folder, report):
    """One stamp, in a Python that has PIL. Returns its own line, or raises.

    The report goes out as a file and comes back as nothing: the stamps take it
    in memory precisely so that `run()` can stamp *before* writing the report -
    see `_stamp` - and a subprocess has no memory to share. So it is written
    beside the atlases under a name of its own and removed again, which keeps
    `_run_report.json` the last write in the directory either way.
    """
    import subprocess
    exe = _pil_python()
    if exe is None:
        raise RuntimeError(
            "no interpreter here can import PIL, so %s cannot run: neither "
            "Blender's own Python nor `py` nor `python`. Run it from the repo "
            "root by hand and say so in the report" % module)
    handover = os.path.join(folder, "_stamp_handover.json")
    with open(handover, "w", encoding="utf-8") as fh:
        json.dump(report, fh, default=str)
    try:
        done = subprocess.run(
            [exe, "-c",
             # One line, no newline escapes: this string travels through a
             # command line and every layer between here and there has an
             # opinion about backslashes.
             "import sys, json, importlib; "
             "sys.path.insert(0, sys.argv[1]); "
             "m = importlib.import_module(sys.argv[2]); "
             "rep = json.load(open(sys.argv[4], encoding='utf-8')); "
             "print(m.stamp(sys.argv[3], report=rep))",
             cfg["repo"], module, folder, handover],
    ...
    if done.returncode != 0:
        raise RuntimeError("%s exited %d: %s"
                           % (module, done.returncode,
                              (done.stderr or "").strip()[-600:]))
    return (done.stdout or "").strip() + " [out of process: %s]" % os.path.basename(exe)


def _stale(cfg, layers, problems):
    """Remove the atlases of layers this scene no longer produces.

    **A layer that stopped being rendered does not stop being read.** The render
    rewrites every `*_atlas.json` it produces and touches nothing else, so a set
    that used to have a layer and now does not keeps the old files - and the
    bench finds a layer by asking whether its file is on disk. It would draw a
    turret that is not on the vehicle, out of frames measured with a different
    `units_per_pixel`, at an anchor from a different fit.

    This is not hypothetical: it is what turning the two casemates into
    casemates does. `turret_atlas` and `wreck_turret_atlas` had been rendered
    into `Sprites/TDP` and `Sprites/HMP` for months, and nothing in a re-render
    would have taken them away.

    Deleted rather than reported, and the argument is the one `_stamp` already
    makes: a line in the report is a thing to remember, and remembering is not a
    mechanism. What is reported is *what was deleted*, so a layer that vanished
    because a piece got un-parented by accident is loud rather than quiet - that
    is the case this could get wrong, so it also raises a `problems` line.
    """
    got = {name for name in layers}
    out = []
    for name in LAYER_ORDER:
        if name in got:
            continue
        for ext in ("json", "png"):
            path = os.path.join(cfg["output_dir"], "%s_atlas.%s" % (name, ext))
            if os.path.exists(path):
                os.remove(path)
                out.append(os.path.basename(path))
    if out:
        problems.append(
            "this render produced no %s layer, so %d stale atlas file(s) were "
            "removed: %s. If that is the change you made, this line is the "
            "receipt; if it is not, a piece has come un-parented and the set is "
            "now missing a layer the bench used to draw."
            % (", ".join(sorted({n.split("_atlas.")[0] for n in out})),
               len(out), ", ".join(out)))
    return out


def _stamp(cfg, report, problems):
    """Re-stamp the set that was just rendered, and say whether it took.

    <b>The render un-stamps what it renders.</b> `sprite_atlas` writes every
    `*_atlas.json` from scratch, and two stamps live inside those files: the
    gun's bore in the shot layers (`stamp_bore`) and the engine port in the
    burning ones (`stamp_ports`). So a re-render of a stamped set drops both,
    and neither loss says anything - the bench keeps drawing, out of its
    fallbacks.

    Measured on 2026-09-09: three re-renders dropped a bore block stamped on
    2026-08-25, and the bench's `FlashSource.Built` silently became the painted
    sheet, whose art seats the muzzle 110px into a 256px cell. It reached me as
    "part of the flash is behind the muzzle" a day later, from someone looking
    at the screen. That is the whole argument for doing it here: it was already
    written down in `/tank` as a step to remember, and remembering is not a
    mechanism.

    Ports before bore, because `stamp_bore.block_for` lifts `hull_length` off
    the ports block rather than fitting that number a second time.

    The stamps are handed the report in memory rather than reading it back,
    which is what lets this run *before* the report is written - so the report
    stays the last thing written in the directory, which is the finish signal
    the polling step relies on, and so the outcome can go into it.
    """
    out = {}
    folder = cfg["output_dir"]
    for name in ("ports", "bore"):
        module = "stamp_" + name
        try:
            out[name] = _load(cfg, module).stamp(folder, report=report)
        except ImportError:
            # Blender has no PIL and both stamps read a PNG. Not an error and
            # not a skip: the same code, in an interpreter that can load it.
            try:
                out[name] = _stamp_apart(cfg, module, folder, report)
            except Exception as exc:                   # noqa: BLE001
                out[name] = "failed out of process: %r" % (exc,)
        except Exception as exc:                       # noqa: BLE001
            out[name] = "failed: %r" % (exc,)
    # And the result, which is not what those two returned. A stamp reports what
    # it wrote; whether the block is in the file afterwards is a different
    # question, and it is the one the bench asks.
    for layer, key, wanted, falls in (
            ("flash", "bore",
             os.path.exists(os.path.join(folder, "barrel_atlas.json")),
             "the shot to the painted sheet, whose art seats the muzzle 110px "
             "into a 256px cell"),
            ("fire", "ports", bool(report.get("exhaust_ports")),
             "the burning engine to the rendered column instead of the "
             "stamped point")):
        path = os.path.join(folder, "%s_atlas.json" % layer)
        got = False
        if os.path.exists(path):
            with open(path, encoding="utf-8") as fh:
                got = bool(json.load(fh).get(key))
        out["%s_in_%s" % (key, layer)] = got
        if wanted and not got:
            problems.append(
                "%s carries no %s block after the render - the bench falls back "
                "silently, %s, so this is the loss that has to be loud: %s"
                % (os.path.basename(os.path.normpath(folder)), key, falls,
                   out.get(key, "no attempt")))
    return out
...
def run(cfg=None):
    ...
    rendered = render(cfg)
    problems = list(rendered["warnings"])
    # **Before `check`, and that is not tidiness.** `check` finds the layers by
    # globbing the directory, so a stale atlas is not merely left behind - it is
    # *read*, and every comparison the check makes against it is against frames
    # from another render with another `units_per_pixel` and another anchor. On
    # the run that turned HM into a casemate that would have meant the wreck
    # sheet drawing last week's turret and the droop being measured against it.
    stale = _stale(cfg, rendered["layers"], problems)
    # between render and check: the plate table is in pixels and needs the
    # camera the render just fitted, and `check` reads it back out of the layer
    # metadata
    ...
    if prepared["muzzle"]:
        report.update({
            ...
            # The direction and the calibre travel with the point now, and this
            # is what `stamp_bore` asked for in writing: without them it derives
            # the direction as `normalize(muzzle.xy - axis.xy)` and pins z to
            # zero "because the gun is level". On HM_SturmTiger the bore sits at
            # 26.5 deg, so that derivation is wrong by 26.5 deg and points the
            # flash out of the side of a tube aimed at the sky. Measured beats
            # derived wherever the measurement exists.
            "muzzle_dir": prepared["muzzle"]["muzzle_dir"],
            "muzzle_elevation": prepared["muzzle"]["muzzle_elevation"],
            "muzzle_radius": prepared["muzzle"]["muzzle_radius"],
            "muzzle_shape": prepared["muzzle"]["muzzle_shape"],
            # how soft the PCA reading was, and how round the piece is about the
            # axis that won - the two numbers that say whether the bore was
            # measured or guessed. See `muzzle_point.refine_axis`.
            "muzzle_axis_refined_deg": prepared["muzzle"]["axis_refined_deg"],
            "muzzle_roundness": prepared["muzzle"]["roundness"],
    ...
    if checked.get("recoil_stroke_px"):
        report.update({
            ...
            "recoil_overshoot_headings":
                checked.get("recoil_overshoot_headings"),
        })
        # And the same for the ladder, in the same breath: laying is a second
        # axis of this layer, so its numbers are dropped by the same omission
        # that dropped these three. Absent on a level gun, which is how a set
        # rendered without laying says so rather than carrying an empty table.
        for key in ("elev_table_deg", "elev_rung_moved", "elev_composite",
                    "elev_clearance_in_radii", "elev_offset_at_cap"):
            if key in checked:
                report[key] = checked[key]
    # Either shape of wreck pose, and asked as "which one is there" rather than
    # as one key: on a casemate the mount does not move and the drooped gun is
    # the pose, so a block keyed on `wreck_turret_moved_px` alone would drop the
    # sheet, the belts and the droop measurement together - which is the exact
    # omission the comment above this one is about.
    moved_key = next((k for k in ("wreck_turret_moved_px",
                                  "wreck_barrel_moved_px")
                      if checked.get(k) is not None), None)
    if moved_key is not None:
        report[moved_key] = checked[moved_key]
        report["wreck_composite"] = checked["wreck_composite"]
        for key in ("wreck_barrel_moved_share", "wreck_void_px",
                    "wreck_void_by_heading", "wreck_track_moved_px"):
    ...
    # Before the report is written, so the report stays the last write in the
    # directory - see `_stamp`, and step 4 of `/tank` on what polls for it.
    report["stamps"] = _stamp(cfg, report, problems)
