...
/// <see cref="TankTick.FateOf"/> knocks a hull out on only one of them - and they
...
public sealed partial class ProcSpall : Node3D
{
    ...
    /// <summary>
    /// Which rung it sorts on - <see cref="ProcKick.Order"/>'s arrangement, its
    /// reason, and the same discovery behind it one layer along.
    ///
    /// <b>Because the clearance alone does not order this against a hull, and
    /// that is measured rather than reasoned.</b> <see cref="Sit"/>'s
    /// <c>behind</c> moves the quads four world units along the view -
    /// <see cref="Stage3D.Clear"/>, written to lift a decal off the cell it lies
    /// on, where four units is the whole depth there is to win. A tank is not a
    /// cell: sprite and fan both draw with <c>depth_draw_never</c>, so nothing
    /// depth-tests and what covers what is the transparent sort inside one rung -
    /// and a fan flipped from one side of a hull to the other by that nudge alone
    /// came out the same picture but for a 36px strip. So the side a fan is on is
    /// said with the rung, which <c>Stage3D</c> states beats depth outright, and
    /// the nudge stays for what it is good for: holding the two halves of this
    /// effect apart.
    ///
    /// Written on every throw, because these come out of a pool -
    /// <see cref="ProcKick.Order"/>'s reason word for word: one dropped a rung
    /// for a ram and handed back would put the next round's fan under the tank it
    /// bounced off.
    /// </summary>
    public int Order
    {
