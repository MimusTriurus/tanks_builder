...
/// A run-up to a sample is a ram the whole way (see <c>WallField.RamAt</c>), so
...
public sealed partial class TankBench
{
    ...
    /// latched off <c>WallField.Named</c> - the event's own answer, left where the
    ...
    {
        if (!_tour || Walls.Count == 0 || _garage.Count == 0 || Wall is null)
            return;
        ...
        // WallField.Named is what it left behind - see WallField.Watch.
        if (_tourAt >= 0 && _tourAt < _tourLegs.Count
            && _bricks.Named is (Vector2I hit, int entered)
        ...
        if (_tourRest < TourPause)
            return;
        bool still = Walls.All(w => w.Rig is null or { Awake: 0 });
    ...
    private WallProp? Prop(Vector2I cell) =>
        Walls.FirstOrDefault(w => w.Cell == cell);
    ...
    private void TourSay(bool still)
    {
        ...
        WallProp? wall = Prop(leg.Wall);
        int at = wall is null ? -1 : _bricks.IndexOf(wall);
        ...
        {
            if (w == at || Walls[w].Rig is not { } other)
                continue;
            int moved = 0;
            for (int s = 0; s < HexField.EdgeHeadings.Length; s++)
                moved += other.Section(s).Gone - _tourWas[w][s];
            if (moved > 0)
                past.Add($"({Walls[w].Cell.X},{Walls[w].Cell.Y}) {moved}");
        }
