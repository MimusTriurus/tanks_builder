...
public sealed partial class TrackMarks : Node2D
{
    ...
    public void Lay(Vehicle v, (double Left, double Right) travel, double delta)
    {
        ...
        // The gauge is the hull's, always: the belts are bolted to it and a
        // shove does not move them about. What the shove changes is the band
        // each of them leaves - see Swath - and that is measured across the
        // push, so the two directions part company here and only here.
        double lie = v.Shoved ?? v.Sprite.HullFacing;
        ...
        double ground = v.Shoved is int push
            ? Swath(v, push)
            : belt * Scrub(v, travel, delta);
        float width = ScreenWidth(v, lie, ground);
        ...
        //
        // <b>A dragged belt leaves none, and the zero is the statement.</b> The
        // ladder is where the shoes bit in, one imprint per link wound over the
        // ground; a locked belt winds nothing, so there is nothing to repeat and
        // what is left is the ribbon alone - which the ladder's own note calls a
        // drag mark, in the sentence that says why a ribbon is not enough for a
        // tank that is driving. See DrawShoes, which skips them.
        Vector2 bar = v.Shoved is null
            ? across * (float)(belt * 0.5)
            : Vector2.Zero;
        ...
        for (int side = 0; side < 2; side++)
        {
            ...
            // <b>A shoved hull measures the ground it covered, not the belt it
            // did not turn.</b> Its belts are locked on purpose - a track wound
            // forward under a tank sliding sideways reads as the tank driving
            // itself, see TankTick.BeltTravel - and taking the cadence from them
            // laid exactly one imprint and then nothing at all for the whole hex
            // a ram threw it across. Measured: against an untouched board, the
            // rammer left two ribbons and the tank it shoved left bare earth.
            // The place was always the hull's; now the pace is too, and it comes
            // off the very point being laid, which this already keeps.
            double moved = v.Shoved is null
                ? Math.Abs(side == 0 ? travel.Left : travel.Right)
                : trail.Down ? at.DistanceTo(trail.LastAt) : 0.0;
    ...
    /// </summary>
    private static float ScreenWidth(Vehicle v, double lie, double ground)
    {
        Vector2 along = v.Atlas.GroundDirection(lie);
        Vector2 across = v.Atlas.GroundDirection(lie + 90.0)
                         * (float)ground;
    ...
    /// <summary>
    /// How wide a band a locked belt sweeps when it is dragged along
    /// <paramref name="push"/>, in ground px.
    ///
    /// <b>Because a belt is a rectangle and a shove can take it across any
    /// diagonal of it.</b> Rolling, a belt leaves its own width and nothing else:
    /// every part of it passes over the same line. Dragged, it leaves the whole
    /// of its footprint smeared along the push, and how wide that is depends on
    /// which way the hull happens to be pointing - the one thing the rut had no
    /// way to say. A tank shoved nose-first leaves two ribbons a belt wide; the
    /// same tank shoved broadside leaves one swath as wide as a belt is
    /// <em>long</em>, because the two bands are then wider than the gauge between
    /// them and merge. That was the picture that was wrong: a hull standing
    /// across the ram drew the two neat ribbons of a hull driving along it.
    ///
    /// The support function of the rectangle across the push, doubled - half a
    /// length times the sine and half a width times the cosine, which is the
    /// rectangle's own shadow on the axis the band is measured across. It is
    /// exact rather than eased: at nought degrees it is the belt's width, at
    /// ninety its length, and nothing in between is a taste.
    ///
    /// <b>And it replaces <see cref="Scrub"/> for this case rather than
    /// multiplying it.</b> That multiplier is a stand-in for ground that is
    /// churned rather than rolled, worked out from how tightly the path bends;
    /// here the geometry says the same thing directly and says it at every angle.
    /// What still separates a drag from a roll at nought degrees is the ladder,
    /// and the drag does not lay one.
    /// </summary>
    private static double Swath(Vehicle v, int push)
    {
        double slant = Mathf.DegToRad(push - v.Sprite.HullFacing);
        return (v.Atlas.TrackLength * Math.Abs(Math.Sin(slant))
                + v.Atlas.TrackWidth * Math.Abs(Math.Cos(slant)))
               * v.Sprite.BodyScale;
    }
    ...
    private void DrawShoes(CanvasItem into, Trail trail, Func<Vector2, bool>? keep)
    {
        ...
        for (int i = 0; i < trail.Points.Count; i++)
            // A bar of no length is a belt that was dragged rather than wound -
            // see Lay. Skipped rather than drawn short: a zero-length segment is
            // a dot of ink at the belt's centre line, which is a shoe imprint
            // claiming to be there.
            if ((keep is null || keep(trail.Points[i]))
                && trail.Bars[i] != Vector2.Zero)
                take.Add(i);
        n = take.Count;
        if (n == 0)
            return;
