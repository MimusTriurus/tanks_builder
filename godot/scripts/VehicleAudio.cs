...
public sealed partial class VehicleAudio : Node2D
{
    ...
    public void Update(Vehicle v, double delta)
    {
        ...
        // Knocked out or destroyed alike: a stopped engine is silent either way,
        // and the knocked-out one is heard stopping - see Stalled.
        bool running = !v.Wreck.Out;
        ...
        // The douse is on the same sentence and for the same reason: while the
        // last of the flame is going out there is still a fire to hear, and it
        // is the flame that says how much - see Vehicle.Ember.
        double flame = v.Sprite.FireDensity;
        Gate(_burn, ref _burnGate,
             Enabled && (v.Burning || v.Ember > 0.0) && flame > BurnFloor, delta,
    ...
    /// <summary>The engine stopping - the class's own <c>engine_stop</c> take,
    /// on the frame a hull is knocked out. The loop's gate closes on the same
    /// frame, so the take is what the ear gets instead of a fade: an engine that
    /// dies away over a second is one being switched off, and this one
    /// stalled.</summary>
    public void Stalled()
    {
        Play(_death, Own["engine_stop"], EngineDb, EventTrim);
    }
