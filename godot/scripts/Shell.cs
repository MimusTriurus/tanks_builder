...
///
/// <b>It used to be a straight line, and the argument for that was sound and is
/// now half of a longer one.</b> An affine projection takes a straight line in
/// the world to a straight line on screen, so a flat trajectory drawn flat is
/// exact - and tank guns at these ranges do shoot flat. What was missing is that
/// a gun laid at a target is not laid <em>on</em> it: the tube stands a little
/// above, and the round comes back down on to the line of sight. So there is an
/// arc on every round now, and how much of one is the class's business - some
/// twenty pixels over five cells for a tank gun (<see cref="Gunnery.SuperDeg"/>)
/// against a couple of hundred for a mortar's bomb (<see cref="Gunnery.LobDeg"/>).
/// The two are the same range equation read at its two roots; see
/// <see cref="Curves"/> and <see cref="Lofted"/>, which is where the difference
/// stops being one of degree.
/// </summary>
public sealed partial class Shell : Node2D
{
    ...
    public Vector2 PuffLocal(int k)
    {
        ...
        // Both points off the one expression, so a puff sits on the path the
        // round actually flew rather than on the straight line between its ends
        // - which is the same point until something is lobbed, and a chord's
        // worth of daylight away as soon as one is.
        Vector2 seat = PointAt(at / total) - PointAt(Flown / total);
        Vector2 along = Heading(at / total);
        Vector2 across = new(-along.Y, along.X);
        float wob = ((k * 41 + Serial * 17) % 23) / 23.0f - 0.5f;
        float t = PuffAge(k) / SmokeSeconds;
        return seat + across * (wob * (1.2f + 5.0f * t) * SmokeSize);
    }
    ...
    /// <summary>Distance covered, never past the target. Along the path rather
    /// than along the chord, which is the same number until something is lobbed
    /// - see <see cref="PathLength"/>.</summary>
    private float Flown => Math.Min(_flown, PathLength);
    ...
    /// <summary>
    /// What the round does when it arrives besides landing: stop, bounce off
    /// along the mirror axis, or go through and on along its own.
    ///
    /// <b>On the shell rather than in the call that fires it</b>, and that is
    /// this file's own rule read once more: what the trigger decided travels
    /// with the round, because the alternative is a round that changes its mind
    /// in flight. The alternative was tried first and does not work at all here
    /// - the event player cannot wait for the arrival and fire the second leg
    /// itself, because with the trail switched off the round is freed on the
    /// frame it lands (see <see cref="Expired"/>) and nothing is left to ask
    /// where the plate was.
    ///
    /// GDD units.md and classes.md are the two rules this is: a non-penetrating
    /// hit on a side plate ricochets on, and a destroyer's round passes through
    /// its first target. Both are one continuation and never two - "второго
    /// рикошета нет" - so the leg this raises is always <see cref="Stops"/>.
    /// </summary>
    public enum Onward { Stops, Bounces, Passes }

    /// <inheritdoc cref="Onward"/>
    ...
    /// round flies on "с той же огневой мощью" and the second tank is judged by
    /// the same matchup as the first, which is exactly what <c>level: null</c>
    /// already means to <see cref="TankTick.Shoot"/>. A number here is for the
    /// cases where the rules changed the round on the way through - the
    /// destroyer's shell, which arrives at its second target one might down.
    /// </summary>
    ...
    /// <summary>
    /// How high over the chord it goes at the top, in screen pixels. Zero is a
    /// tank gun and is the default: the flat round is not a lob with the arc
    /// turned off, it is what every gun but one fires.
    ///
    /// <b>A height and not a fraction of the chord, because the tube decides the
    /// arc.</b> It is worked out where the angles are - in ground units, off the
    /// range in cells and the angle the gun is laid at - and projected exactly
    /// once on the way here: <see cref="Gunnery.ApexPx"/>. As a fraction of the
    /// drawn chord the same gun would read as two machines depending on which
    /// way it happened to be pointing, the board squashing one screen axis and
    /// not the other - and, worse, the arc would be a number of its own beside
    /// the tube's angle with nothing keeping the two in step.
    ///
    /// <b>Nearer is steeper falls out of it.</b> The angle is solved against the
    /// range the bomb has to cover, so the arc is solved with it - see
    /// <see cref="Gunnery.LobDeg"/>. That is what a gunner cranking a mortar up
    /// at a close target is doing, and it is the reason the tube's ladder and
    /// this cannot be two independent numbers.
    ///
    /// <b>Settled at the trigger, like everything else about the round.</b> A
    /// bomb in the air does not re-solve its own trajectory because the tank it
    /// is falling on drove a little.
    /// </summary>
    public float Apex { get; init; }

    /// <summary>Whether its path is a curve at all rather than a chord. True of
    /// every round a gun lays above its own line of sight, which since the tube
    /// started being laid is nearly all of them - see
    /// <see cref="Gunnery.SuperDeg"/>. Read rather than carried: the apex is the
    /// fact, and a bool beside it would be the same thing said twice.</summary>
    public bool Curves => Apex > 0.0f;

    /// <summary>
    /// Whether it goes <em>over</em> things rather than merely being bent - GDD
    /// classes.md, "HM — Навес".
    ///
    /// <b>Two predicates and not one, and the difference is what a shadow is
    /// for.</b> Every gun on the board now throws some arc: a direct one is laid
    /// a few degrees above its target to counter the drop, and over five cells
    /// that is a bow of some twenty pixels - a curve, and nothing that leaves the
    /// ground. Draw a running shadow under that and the board sprouts a mark
    /// under every shell; swell the tracer by a third for it and a tank gun's
    /// round pulses for no reason.
    ///
    /// <b>The threshold is measured rather than picked: the round clears its own
    /// shooter's roof.</b> That is exactly the height at which "over" starts to
    /// mean anything on this board - a round below it passes through the same
    /// space a hull occupies, and one above it is visibly over the top of one.
    /// A tuned number of pixels would have been the same claim with nothing
    /// behind it.
    /// </summary>
    public bool Lofted => Apex > Shooter.Atlas.DrawnHeight * Shooter.Sprite.BodyScale;

    /// <summary>
    /// Whether the <em>rules</em> treat it as arriving from above: no armour
    /// zone, no plate to bounce off, and nothing between shooter and hex that
    /// can stop it - GDD classes.md, "снаряд приходит сверху... минуя всё, что
    /// стоит между стрелком и гексом", and units.md, "для навесного огня
    /// миномёта зона значения не имеет".
    ///
    /// <b>The class and not the height, and the difference has already cost
    /// once.</b> This used to be read off the apex, which was a mortar's alone
    /// until the tube started being laid - and the moment a direct gun got its
    /// own superelevation every tank round on the board began aiming at roofs,
    /// losing its plate and refusing to bounce. An apex is a fact about one
    /// shot; whether a gun shoots over things is a fact about the gun.
    ///
    /// So there are three questions here and they are three: <see cref="Curves"/>
    /// is the path, <see cref="Lofted"/> is whether it is drawn as going over
    /// something, and this is what the rules do about it.
    /// </summary>
    public bool Overhead => Shooter.Profile.Lobs;

    /// <summary>
    /// The shape of the throw: 1 at the top, 0 at both ends.
    ///
    /// A parabola and not a sine, and it is the one of the two that is the
    /// event rather than a curve that looks like it. A round under gravity with
    /// its horizontal speed unchanged draws exactly this, which is also why the
    /// parameter may be advanced at a constant rate (see <see cref="Advance"/>)
    /// - uniform in <c>t</c> is uniform along the ground, and that is what the
    /// physics says, not an approximation of it. The sine is a fifth of a
    /// pixel from it at the top and wrong at the ends, where it leaves flatter
    /// than gravity would.
    /// </summary>
    public static float Hump(float t) => 4.0f * t * (1.0f - t);

    /// <summary>How high over the chord it stands a fraction
    /// <paramref name="t"/> of the way along.</summary>
    public float HeightAt(float t) => Apex * Hump(Math.Clamp(t, 0.0f, 1.0f));

    /// <summary>How high it stands now.</summary>
    public float Height => Curves ? HeightAt(Fraction) : 0.0f;

    /// <summary>How high it stands as a share of its own apex: 0 on the ground
    /// at both ends and 1 at the top. What the streak's size and the shadow's
    /// are both read off, so the two say the same thing about the same
    /// round.</summary>
    public float High => Curves ? Hump(Fraction) : 0.0f;

    /// <summary>
    /// The board point directly below it, on the datum - where its shadow goes.
    ///
    /// <b>Two subtractions and both of them are the round's own.</b> The arc is
    /// taken back off, which puts the point on the chord; then the chord's own
    /// lift is taken off, which puts it on the ground. That second one is the
    /// half worth stating: the chord does not run along the ground, it runs from
    /// a muzzle some sixty pixels up to a plate or a deck that is up as well, so
    /// a shadow seated on the chord would float by the muzzle's height at one
    /// end and the deck's at the other.
    ///
    /// <see cref="Vehicle.LiftOf"/> measures from the datum rather than from the
    /// cell, so what is left here is a datum-level point and the ground's own
    /// rise under it is the board's to add - see <c>Stage3D.Shades</c>.
    /// </summary>
    public Vector2 Under
    {
        get
        {
            ...
            return PointAt(t)
                   + new Vector2(0.0f,
                                 HeightAt(t) + Mathf.Lerp(FromLift, ToLift, t));
        }
    }

    /// <summary>
    /// Where it is a fraction <paramref name="t"/> of the way along, in board
    /// space.
    ///
    /// <b>One expression, and everything that draws the round goes through
    /// it</b> - the head, the trail, and the shadow the stage puts under it.
    /// Written out at each of those, an arc would have three chances to
    /// disagree about where the round is, and two of them are only visible in
    /// motion.
    ///
    /// Height is minus y and nothing else, which is what the board's own lift
    /// already means: <see cref="Vehicle.LiftOf"/> measures screen pixels of
    /// height above a contact row, and the round carries such a lift at both
    /// ends already. The flat case is the chord to the bit - the same
    /// <c>Lerp</c> that was here before the arc was.
    /// </summary>
    public Vector2 PointAt(float t)
    {
        Vector2 flat = From.Lerp(To, t);
        return Curves ? flat - new Vector2(0.0f, HeightAt(t)) : flat;
    }

    /// <summary>
    /// Which way it is pointing a fraction <paramref name="t"/> along: the
    /// tangent to the path, as a unit screen vector.
    ///
    /// <b>The tangent and not the chord, and that is the whole of what a still
    /// frame cannot tell you.</b> A lance drawn along the chord on a round
    /// flying an arc is a shell travelling one way and pointing another - right
    /// at the apex, wrong by the departure angle at both ends, and invisible in
    /// every screenshot.
    /// </summary>
    public Vector2 Heading(float t)
    {
        Vector2 chord = To - From;
        if (!Curves)
            return chord.LengthSquared() < 1e-6f
                ? Vector2.Right : chord.Normalized();
        // d/dt of PointAt: the chord, less the rate the height is climbing.
        var along = chord - new Vector2(0.0f, Apex * (4.0f - 8.0f * t));
        return along.LengthSquared() < 1e-6f ? Vector2.Right : along.Normalized();
    }

    /// <summary>How many segments the path is measured in. Sixteen: the length
    /// it returns is within a tenth of a pixel of the closed form at the arc
    /// this throws, and it is read once a frame per round.</summary>
    private const int PathSteps = 16;

    /// <summary>
    /// How far the round actually travels, in pixels - the chord for a flat
    /// round and the arc for a lobbed one.
    ///
    /// <b>This is what makes the lob take longer, and it is the second half of
    /// what makes it a lob.</b> Speed is one figure for every gun in the game
    /// (see <see cref="BaseSpeed"/>), so the only honest way for a mortar round
    /// to hang in the air is for it to have further to go - which it does. At
    /// the tuned arc the path is about a fifth longer than the chord, and the
    /// range it is fired over is two to three times the bench's standoff on top
    /// of that.
    ///
    /// Sampled rather than solved: the closed form for a parabola's length is
    /// three transcendental terms, it would have to be re-derived if the shape
    /// ever changes, and what it buys is a tenth of a pixel.
    /// </summary>
    public float PathLength
    {
        get
        {
            ...
            if (!Curves || chord < 1e-3f)
                return chord;
            float run = 0.0f;
            Vector2 was = PointAt(0.0f);
            for (int i = 1; i <= PathSteps; i++)
            {
                Vector2 now = PointAt(i / (float)PathSteps);
                run += (now - was).Length();
                was = now;
            }
            return run;
    ...
    /// <summary>
    /// How much bigger the streak is drawn at the top of its arc.
    ///
    /// <b>The one cue that says "up" in a picture with no horizon in it.</b> A
    /// round climbing over a wood on this camera moves up the screen, and up
    /// the screen is also what a round flying away from the viewer along the
    /// ground does - the two are the same movement drawn twice unless something
    /// else separates them. Nearer the eye is bigger, so the streak grows with
    /// the height and shrinks back into the ground where it lands.
    ///
    /// Modest on purpose: a third again at the top reads as height, and twice
    /// over reads as a second round.
    /// </summary>
    public const float Loom = 1.35f;
    ...
    public void Advance(double delta)
    {
        ...
        Vector2 to = To;
        float lift = ToLift;
        // The path and not the chord, which is the same length until something
        // is lobbed - see PathLength, where the difference is the flight time.
        float total = PathLength;
        ...
        {
            // Uniform in the parameter, which for a parabola is uniform along
            // the ground - see Hump. So a lobbed round crosses the board at an
            // even rate and is slowest on screen at the top of its arc, which is
            // what a thrown thing does.
            Position = PointAt(_flown / total);
        }
        QueueRedraw();
    }
    ...
    public override void _Draw()
    {
        ...
        if ((to - From).LengthSquared() < 1e-6f)
            return;
        float at = Fraction;
        // Along the tangent, which on a flat round is the chord to the bit and
        // on a lobbed one is the only direction that is not a lie - see Heading.
        Vector2 along = Heading(at);
        // Bigger up high, which is the one cue on this camera that says the
        // round is over the board rather than further down it - see Loom.
        float cal = TracerSize
                    * (Lofted ? 1.0f + (Loom - 1.0f) * Hump(at) : 1.0f);
