...
public sealed partial class ProcSmoke : Node2D
{
    ...
    /// <summary>
    /// Whether a root opens with the column built rather than read.
    ///
    /// <b>True: the built track is the game's, decided 2026-09-07</b>
    /// (docs/effects-plan.md, "Дорожка по умолчанию"). It was false while the
    /// pair was being judged - every atlas ships the rendered column, and an
    /// A/B has to start from what is shipped - and the judging is done: the
    /// rendered column steps twelve phases and fifteen degrees beside a forest
    /// that burns continuously, and costs a twelve-minute render per tweak
    /// (docs/motion.md). The rendered layers stay loaded and stay the other
    /// side of the A/B; the panel rows and <c>--no-proc-*</c> bring them back.
    /// Named here rather than left in a field initialiser for
    ...
    /// <summary>The knocked-out hull's smoke: oil and coolant on a hot engine,
    /// grey rather than the wreck's soot - between the column's ink and the
    /// plume's, and nearer the plume. See <see cref="Wreck.Disabled"/>.</summary>
    public static readonly Color SmoulderInk = new(0.455f, 0.445f, 0.437f);

    /// <summary>
    /// Steam: what a fire that has just been put out leaves behind.
    ///
    /// <b>The third ink, and the only one that is not a shade of the other
    /// two.</b> Soot and smoulder are dark and warm-grey because they are what
    /// is left of burnt paint; this is water, and it goes the other way - near
    /// white and faintly cool, which is <c>SheetBlast.FoamStops</c>'s own
    /// argument at the other end of the board. Read against the column it
    /// replaces, that is the whole message: the same shape of cloud off the
    /// same ports, pale instead of black, means the fire is out rather than
    /// dying down.
    /// </summary>
    public static readonly Color SteamInk = new(0.878f, 0.898f, 0.918f);

    public float ToneVary = 0.08f;

    /// <summary>
    /// How much bigger than its own model this cloud is drawn - 1 for the
    /// column as it is tuned.
    ///
    /// <b>For the steam a doused fire leaves, and it is a multiplier rather
    /// than a second set of dials on purpose.</b> What comes off a hull that
    /// has just been put out is the same cloud from the same ports - same
    /// ports, same drift, same lifetimes - and the two things that differ are
    /// its colour (<see cref="SteamInk"/>) and that there is briefly more of
    /// it. A second model would be a fifth hand-written plume wearing this
    /// one's name, which is the mistake SheetBlast's two ramps are the answer
    /// to at the other end of the board.
    ///
    /// On the size of a puff and on how far it wanders, which together are the
    /// cloud: the path, the buoyancy and the lifetimes are untouched, so it
    /// swells without rising faster or hanging longer. Written per frame by
    /// <c>TankTick.UpdateBurn</c>, beside the ink and for the same reason.
    /// </summary>
    ...
    private bool Running => Tank is not null
        && (Plumed ? Tank.ExhaustPhase >= 0 && Tank.ProceduralExhaust
                   : (Tank.Burning || Tank.Smouldering) && Tank.ProceduralSmoke);
    ...
    public Puff[] Build(AtlasSet.Port port, double hullLength, float cycle)
    {
        ...
        for (int k = 0; k < count; k++)
        {
            ...
            float wander = hull * Spread * Swell * MathF.Pow(age, 0.7f)
                           * Plumes.Hash01(k, Seed + 3);
