...
public sealed partial class Grove : Node2D
{
    ...
    /// <remarks><b>And never where the heavy has been</b>, which is the one
    /// refusal here that is not about the ground: a wood a bulldozer drove into
    /// is spent (<see cref="HexField.Felled"/>), and the paint under it still
    /// says forest because becoming ground is a fact about the cover and not a
    /// repaint. Asked here rather than written into the kinds so that a board
    /// which authored no kinds - where the woods are a hash - can be cleared a
    /// cell at a time without the hash having to be argued with.</remarks>
    ...
    public IEnumerable<(PropTier Tier, int Least, int Most)> Growth(Vector2I cell)
    {
        if (Props is null || !Grows(cell) || Field is null)
            yield break;
        // A wood the heavy flattened carries what any ground carries - the
        // scatter of bushes and stones its neighbours have - rather than
        // nothing. Nothing would be a bald hexagon in the middle of a scattered
        // board, which is a scar where the rules say there is a field; and the
        // forest row would put the trees straight back on the next sowing.
        string kind = Field.Felled(cell) ? AnyGround : Field.KindAt(cell);
    ...
    /// <summary>
    /// Put the wood on one cell over: a bulldozer drove into it -
    /// <see cref="TankTick.UpdateWood"/>, GDD classes.md "Бульдозер HT".
    ///
    /// <paramref name="along"/> is the way the hull is going, in the field's own
    /// screen px, and it decides which way the crowns go: what puts a tree down
    /// is the glacis, so they go over ahead of the tank and not away from it the
    /// way <see cref="Brush"/> parts them.
    ///
    /// <b>These props and no others.</b> Re-sowing the board would be the
    /// obvious way to say "this cell has no trees now" and it is the wrong one:
    /// the lattice is walked tier by tier and the undergrowth tests against
    /// whatever is already standing (see <see cref="Plant"/>), so taking one
    /// cell out of the ground and sowing again can move bushes two cells away.
    /// A felling is one cell's business and nothing else on the board is allowed
    /// to move.
    /// </summary>
    /// <b>The trees and not the scatter.</b> What makes a cell a wood is what
    /// stands on it that a fire can carry (<see cref="PropTier.Carries"/>), and
    /// that is what goes down; the bushes and stones stay where they are. A cell
    /// stripped of all three would be a bald hexagon in a scattered field - a
    /// scar where the rules say there is ground - and the rules say the hex
    /// becomes ground, not that it becomes bare.
    ///
    /// <b>Told, not moved.</b> Nothing is freed here: the props are given their
    /// two clocks and <see cref="Blow"/> runs them, because falling over and
    /// then fading out is what the event looks like and a prop removed on the
    /// frame the board changed would be a wood that blinked.
    /// </summary>
    /// <remarks>
    /// <b>Told one at a time as the hull comes up to them, not all at once when
    /// the board changed.</b> The cell going to <c>Cleared</c> is the rules
    /// answering a question about the hex, and it happens on the drive in;
    /// trees are things standing in particular places, and a trunk two thirds
    /// of a hex ahead of the glacis has nothing pushing it over yet. Called
    /// every frame the hull is on the cell with the hull's own ground point,
    /// this reaches each trunk when the tank does - which is the picture, and
    /// it staggers the wood for free and better than a dice roll could.
    ///
    /// <paramref name="hull"/> is where it is, in field-local screen px, and
    /// <paramref name="reach"/> how far from there the wood goes down, in ground
    /// px - the same pair and the same projection <see cref="Brush"/> measures
    /// with, y over the squash so a radius on the ground is a radius. Null hull
    /// is the whole cell at once, which is what a test asking about the pose
    /// wants and what nothing driving should use.
    /// </remarks>
    /// <returns>How many props were told to go over.</returns>
    public int Topple(Vector2I cell, Vector2 along, Vector2? hull = null,
                      double reach = 0.0, double shove = 1.0)
    {
        if (Field is null)
            return 0;
        // Which way they go: along the hull's travel, because what puts them
        // down is the glacis. A shear draws no direction but sideways - the
        // silence Brush names - so a hull driving straight up or down the screen
        // has no way to say "over there", and then the wood parts instead: each
        // trunk goes over on the side of the cell it stands on.
        Vector2 way = along.Normalized();
        bool parts = Math.Abs(way.X) < 0.35f;
        Vector2 middle = Field.CellCentre(cell);
        float squash = Squash;
        Vector2? near = hull is null
            ? null
            : new Vector2(hull.Value.X, hull.Value.Y / squash);
        int going = 0;
        foreach (PropNode prop in _planted)
        {
            if (prop.Cell != cell || !prop.Tier.Carries || prop.Going)
                continue;
            if (near is not null
                && near.Value.DistanceTo(
                       new Vector2(prop.Ground.X, prop.Ground.Y / squash))
                   >= reach)
                continue;
            double roll = Hash01(prop.Seed, prop.Species, 907_003);
            float side = parts
                ? (prop.Ground.X >= middle.X ? 1.0f : -1.0f)
                : Math.Sign(way.X);
            prop.Going = true;
            prop.Waits = roll * FallStagger;
            // Off its feet by the glacis and no further: the rest is the
            // pendulum's - see Timber. A shove rather than an angle, because
            // how hard a hull throws a tree is how fast it is going, and an
            // angle would be the same tree however it was hit.
            prop.Spin = (float)(FallShove * (0.6 + 0.4 * Math.Clamp(shove, 0.0, 1.0))
                                * (0.85 + 0.3 * roll));
            // Not all the same distance over: a rank of trunks stopping at one
            // angle is the lockstep the stagger is there to avoid, said again in
            // the pose instead of the timing.
            prop.Falls = (float)(side * (0.85 + 0.3 * roll));
            prop.Turn = (float)(side * FallLift);
            going++;
        }
        _razed |= going > 0;
        return going;
    ...
    /// <summary>
    /// One trunk's frame of going over: the pendulum, the landing and what the
    /// crown does about both. Returns the lean the branches are being dragged
    /// to, for <see cref="Blow"/> to feed the flinch spring; the landing's kick
    /// goes straight into that spring's rate, because it is an impulse and not a
    /// pull.
    ///
    /// <b>Four phases, and only the second is drawn by anything but this.</b> It
    /// hangs at a few degrees while the hull leans on it; the root plate lets go
    /// and it comes over as <c>a'' = k*sin(a)</c>, slow then quick; the crown
    /// hits and the butt bounces (<see cref="FallBounce"/>) while the branches
    /// are thrown on past the trunk; and then it lies there, and nothing is
    /// moving but the crown ringing down. The fade waits for that - see
    /// <see cref="CrownQuiet"/>.
    ///
    /// <b>Worked on the magnitude, with the side carried separately.</b>
    /// <c>sin(a)</c> of a signed angle would pull a trunk that bounced back past
    /// upright the wrong way; the side a tree goes over on was decided once, by
    /// <see cref="Topple"/>, and nothing here may change it.
    /// </summary>
    private float Timber(PropNode tree, double delta)
    {
        if (tree.Waits > 0.0)
        {
            tree.Waits -= delta;
        ...
        float side = tree.Falls < 0.0f ? -1.0f : 1.0f;
        double rest = Math.Min(Mathf.DegToRad(FallAngle) * Math.Abs(tree.Falls),
                               Mathf.DegToRad(FallFlat));
        double drag = 0.0;
        if (!tree.Lying || tree.Spin != 0.0f)
        {
            double angle = Math.Abs(tree.Turn);
            double pull = FallGravity * Math.Sin(angle);
            tree.Spin = (float)(tree.Spin + pull * delta);
            angle += tree.Spin * delta;
            if (angle >= rest)
            {
                angle = rest;
                double hit = tree.Spin;
                // The ground goes up once, on the first touch. A trunk touches
                // three times - the landing and two bounces - and a cloud per
                // touch is the same dust thrown three times from one pool, which
                // is what it looked like: a puff that kept restarting. The
                // bounces are drawn by the trunk's own pose and need no dirt.
                if (!tree.Lying)
                    Thud?.Invoke(tree.Cell,
                                 tree.Ground + new Vector2(
                                     (float)(side * tree.Rise * Math.Sin(rest)),
                                     0.0f),
                                 new Vector2(side, 0.0f));
                tree.Lying = true;
                // The trunk stops and the branches do not: an impulse to the
                // spring, in the direction it was going, where the drag below
                // is a pull against it.
                tree.FlinchRate += (float)(CrownWhip * side * hit);
                tree.Spin = Math.Abs(hit) < FallSettle
                    ? 0.0f : (float)(-hit * FallBounce);
            }
            else if (angle < 0.0)
            {
                angle = 0.0;
            ...
            tree.Turn = (float)(side * angle);
            tree.Down = (float)Math.Min(1.0, angle / Math.Max(1e-6, rest));
            // What the branches are trailing by while the trunk is under way.
            drag = -CrownLag * side * pull;
        }
        // And going out, once the trunk is still and the crown has stopped
        // arguing about it.
        if (tree.Lying && tree.Spin == 0.0f
            && Math.Abs(tree.Flinch) < CrownQuiet)
            tree.Gone = (float)Math.Min(
                1.0, tree.Gone + delta / Math.Max(1e-3, FadeFor));
        return (float)drag;
    }

    /// <summary>
    /// A crown reaching the ground: the cell it belongs to, where it landed and
    /// which way the trunk is pointing, all in the field's own flat px.
    ///
    /// <b>Raised from here rather than thrown by whoever felled it</b>, because
    /// the moment the dust belongs to is a second and a half after the hull drove
    /// in and the tank has long since stopped: the tick knows when a wood was
    /// spent, and only the wood knows when a tree hit the ground. One per trunk,
    /// at the crown's own point.
    /// </summary>
    public Action<Vector2I, Vector2, Vector2>? Thud;

    /// <summary>Whether a bulldozer has taken anything out of this wood since it
    /// was sown.</summary>
    private bool _razed;

    /// <summary>
    /// Put back what a bulldozer flattened - what <c>R</c> owes the wood.
    ///
    /// <b>Only when something went down</b>, because sowing again rebuilds every
    /// billboard on the board (<c>Stage3D.BuildTrees</c> counts sowings, not
    /// props) and a reset on an untouched board should cost nothing. The sowing
    /// itself is deterministic, so what comes back is the same wood that stood
    /// there - but it comes back green and upright, which is what a reset is.
    ///
    /// <b>After the covers are restored, never before.</b> A cell still carrying
    /// <see cref="CoverState.Cleared"/> is one <see cref="IsForest"/> refuses to
    /// sow, so a re-sow run first would put the board back with a bald hexagon
    /// in it.
    /// </summary>
    /// <returns>Whether anything was re-sown.</returns>
    public bool Regrow()
    {
        if (!_razed)
            return false;
    ...
    /// <summary>
    /// How far from a hull's ground point a trunk goes over, in ground px.
    ///
    /// <b>Wider than a hull looks, because the point it is measured from is the
    /// middle of one.</b> A heavy is about 190 ground px long, so its glacis is
    /// some 95 ahead of the ground point everything here is measured from, and a
    /// trunk goes over when the glacis is against it rather than when it is
    /// against the trunk's own axis - a crown's half width again. Measured on
    /// the events board: a hull stopped dead on a cell's centre is 83 or more
    /// from the nearest trunk (see <see cref="KeepOut"/>) and up to about 125
    /// from the farthest, and at 100 it stood in the middle of a wood it had
    /// cleared with every tree of it upright.
    ///
    /// <b>What it buys is the order, not the count.</b> A heavy entering a hex
    /// has its ground point a good 50px outside it, so the trees on the near
    /// side go over as it crosses the rim and the far side waits for it to get
    /// there - which is the whole point of measuring at all, and it is what the
    /// wood looked like it was doing wrong when the whole cell went over on the
    /// frame the board changed.
    ///
    /// <b>A hull that stops short leaves what it never reached.</b> The hex is
    /// ground either way - that is the board's answer and it was written on the
    /// drive in - and a few trunks still standing at the corners of a hex a tank
    /// only clipped is what that looks like rather than a bug in it.
    /// </summary>
    public double FellReach = 140.0;

    /// <summary>How far past the glacis a root still counts as being under the
    /// tank, in hull half-lengths.
    ///
    /// <b>Forward only, and small.</b> A log the tank is about to be standing on
    /// is the tank's ground as much as the log under its belly is - that is what
    /// "a root on the tank's path" means - and half a hull ahead is the ground
    /// the glacis takes in about a second. Backwards it is not stretched at all:
    /// what the tracks have already passed is behind the hull, and a log behind
    /// a tank is a log in front of the camera.</summary>
    public double FellUnder = 0.5;

    /// <summary>
    /// What pulls a trunk over, in radians a second squared at ninety degrees -
    /// the <c>k</c> of <c>a'' = k*sin(a)</c>.
    ///
    /// <b>A pendulum and not a clock.</b> A tree hinged at its root plate is one,
    /// and the shape it gives is the reason for doing it this way: at three, a
    /// trunk started at <see cref="FallLift"/> with an ordinary shove spends
    /// about 0.6 s in the first twenty degrees and about 0.5 s in the last sixty,
    /// and lands at around one and a half seconds. A hand-shaped curve has to be
    /// told all of that and then tells every tree the same story.
    /// </summary>
    public double FallGravity = 3.0;

    /// <summary>How hard the glacis starts it, in radians a second, at a hull's
    /// cruising speed. Scaled by how fast the tank is actually going, because
    /// that is the one thing about a felling a clock cannot express: a heavy
    /// flat out throws a tree over, a heavy nosing into it leans on it.</summary>
    public double FallShove = 0.35;

    /// <summary>The angle a shoved trunk starts from, in radians. Not zero,
    /// because <c>sin(0)</c> is zero and a tree standing exactly upright would
    /// have nothing but the shove to get it going - three degrees is below
    /// seeing and puts the pendulum on its own feet.</summary>
    public double FallLift = 0.05;

    /// <summary>How much of its rate a trunk keeps when the crown hits the
    /// ground. A tree that lands does not stop dead: the butt comes up a few
    /// degrees and it settles in a bounce or two. Below
    /// <see cref="FallSettle"/> of a bounce there is nothing left to draw and
    /// the trunk is done.</summary>
    public double FallBounce = 0.3;

    /// <summary>The rate below which a bounce is not worth having, in radians a
    /// second.</summary>
    public double FallSettle = 0.25;

    /// <summary>
    /// How far the crown lags the trunk, as lean per radian a second squared of
    /// the trunk's own acceleration.
    ///
    /// <b>The branches are the one part of this that is not rigid, and the shear
    /// is exactly the word for it.</b> <see cref="PropNode.Lean"/> means "the
    /// top of the picture is offset from the foot by its height" - which is what
    /// a crown doing something its trunk is not doing looks like. So the turn
    /// carries the trunk and the lean carries the branches, each saying the
    /// thing it already meant, and the crown trails while the trunk picks up
    /// speed.
    ///
    /// <b>Fed into the flinch spring rather than written.</b> Lean has exactly
    /// one writer (see <see cref="Blow"/>), and the spring the blast and the
    /// hull already drive is the right one: it rings at about three hertz and
    /// dies in a third of a second, which is a branch.
    /// </summary>
    public double CrownLag = 0.013;

    /// <summary>How hard the crown is thrown on when the trunk stops, as a kick
    /// to the flinch spring per radian a second of the landing. This is the half
    /// of the effect anybody watching a tree come down actually remembers: the
    /// trunk is still and the branches are not, for the better part of a second
    /// after.</summary>
    public double CrownWhip = 0.35;

    /// <summary>How quiet the crown has to be before the trunk starts fading,
    /// in lean. A trunk that began going out while its own branches were still
    /// swinging would be saying the two are unrelated.</summary>
    public double CrownQuiet = 0.015;

    /// <summary>How long a fallen trunk lies there before it is gone, in
    /// seconds. Long on purpose, and asked for in as many words: the cell is
    /// ground from the frame the board was written, and the picture takes its
    /// time agreeing.</summary>
    public double FadeFor = 3.5;

    /// <summary>How far apart the trunks on one cell start going over, in
    /// seconds. Small, and smaller than it was: what spreads a wood out now is
    /// the hull arriving at one trunk before the next (<see cref="Topple"/>),
    /// and this is only there so that two trees the tank reaches on the same
    /// frame do not move as one.</summary>
    public double FallStagger = 0.25;

    /// <summary>How far over a fallen trunk ends up, in degrees from upright.
    /// Not ninety: a trunk exactly flat has no drawn height at all and its art
    /// closes to a line, and a wood lying at eighty-odd degrees still reads as
    /// down while keeping the crown on the ground where it can be seen.</summary>
    public double FallAngle = 82.0;

    /// <summary>The angle no trunk is allowed past, in degrees from upright.
    ///
    /// <b>Ninety is not a limit that can be approached, it is a cliff.</b> The
    /// spread on <see cref="PropNode.Falls"/> is a share either side of one, so
    /// the furthest trees were stopping at ninety-four - and past ninety the
    /// card's own plane tips <i>under</i> the board: every point of the trunk is
    /// below the ground plane, the ground is opaque and writes depth, and the
    /// trunk is eaten from the butt up. Measured on the event bench: about half
    /// the length of the nearest trunk gone, which is the cut this cap exists to
    /// answer - the other half of it was a burial that has been taken out, see
    /// <see cref="Stage3D.Rooted"/>.
    ///
    /// Below <see cref="FallAngle"/> would silence the spread altogether, so it
    /// is above it: the trees that roll high stop here together, the rest stop
    /// where they rolled.</summary>
    public double FallFlat = 88.0;
    ...
    public void Blow(double delta)
    {
        ...
        foreach (PropNode tree in _planted)
        {
            ...
            if (!tree.Sways)
                continue;
            float lean = 0.0f;
            // The felling first, because the crown's own spring is driven by it:
            // what the branches lag behind is the trunk's acceleration, and what
            // throws them on is the trunk stopping. Returns the lean the crown
            // is being dragged to this frame; the landing's kick it writes
            // straight into the spring's rate.
            float drag = tree.Going ? Timber(tree, delta) : 0.0f;
            if (Wind > 0.0)
            {
            ...
            if (tree.Flinch != 0.0f || tree.FlinchRate != 0.0f || drag != 0.0f)
            {
            ...
            // The wind goes out as the trunk goes over - a tree on its way down
            // is not swaying - and the crown's own spring does not: it is the
            // only thing still moving once the trunk is lying there. Written as
            // one expression so that a prop nobody felled gets the sum it always
            // got, to the bit.
            float over = tree.Going ? tree.Down * tree.Down : 0.0f;
            lean = lean * (1.0f - over) + tree.Flinch;
            ...
            if (tree.Going)
                // The fade is not in Lean, so the redraw test below cannot see
                // it: a trunk lying still while it goes out would stop being
                // drawn on the frame it landed.
                tree.QueueRedraw();
    ...
    /// <summary>
    /// Take away the trunks that have finished fading - <see cref="Topple"/>'s
    /// second half, run after the leans because a prop freed inside that loop
    /// would be one the enumerator is standing on.
    ///
    /// <b>One sowing counted for the lot.</b> The stage rebuilds its billboards
    /// when <see cref="Sown"/> moves, and a cell's worth of trunks going out
    /// together is one change to the wood, not eight.
    /// </summary>
    private void Gather()
    {
        int gone = 0;
        ...
        {
            PropNode prop = _planted[i];
            if (!prop.Going || prop.Gone < 1.0f)
                continue;
            _planted.RemoveAt(i);
            RemoveChild(prop);
            prop.QueueFree();
    ...
    /// <summary>
    /// A round bursting in the wood itself. Between the two above, and both
    /// halves of that are a claim.
    ///
    /// <b>Over <see cref="HitBlast"/>, because nothing is spent getting there.</b>
    /// A hit is a round breaking up against a plate at the tree line and shoving
    /// the wood from outside it; this one goes off among the crowns, and every
    /// tree on the cell is at the source.
    ///
    /// <b>Under <see cref="DeathBlast"/>, because it is one shell.</b> A tank
    /// going up is the machine itself letting go, and a wood that answered a
    /// single round as hard would be saying the two are the same event.
    /// </summary>
    public double TreeBlast = 12.0;
    ...
    /// the band and the ghost ask "is a hull on this prop's ground" - the
    /// third set, the wood a bulldozer has claimed, asks something else and
    /// is answered on its own below. That predicate is not cheap to
    ...
    public void Reveal(IReadOnlySet<Vector2I> occupied, double delta,
                       IReadOnlySet<Vector2I>? razing = null,
                       IReadOnlyList<Footing.Tread>? treads = null)
    {
        if (_planted.Count == 0)
            return;
        ...
        foreach (PropNode tree in _planted)
        {
            ...
            //
            // <b>A felled trunk is not dressing, and the band is not where it
            // was put.</b> It went there because a crown laid across the hull
            // that knocked it down read as a tree drawn through a tank - but the
            // band is for good, and what it buys on the far side of the hull it
            // pays for on the near one: every trunk lying between the camera and
            // the tank was drawn under it, which is a tank standing on top of a
            // wood it has just flattened. The foot is the rule the whole board
            // sorts by and it answers both: a trunk whose foot is behind the
            // hull goes behind it, crown and all, and one lying in front of the
            // hull covers the tracks, which is what a log in front of a tank
            // does.
            // <b>And a trunk the tank is standing on goes under it, whoever
            // knocked it down.</b> Measured at the <b>root</b> against the
            // <b>hull's own box</b> (<see cref="Footing.Tread"/>, the pair of
            // ground axes a mine under a tank is found with), and not by any
            // radius: a root outside the sprite is a tree in front of the tank
            // and belongs over it, a root under the sprite is ground the tank is
            // standing on and belongs under it. The box is stretched forward by
            // <see cref="FellUnder"/> and never backwards, because what the
            // glacis is about to be standing on is the tank's too - that is the
            // path in "a root on the tank's path".
            bool trodden = false;
            if (tree.Going && treads is not null)
                foreach (Footing.Tread tread in treads)
                    if (tread.Covers(tree.Ground, out _, (float)FellUnder))
                    {
                        trodden = true;
                        break;
                    }
            bool dressed = (tree.Tier.UnderTanks && under) || trodden;
            ...
            // <b>A trunk that is going over is left exactly as it is.</b> The
            // reveal used to want it back at full strength - a felled tree is
            // leaving on its own clock, and ghosted as well the one picture this
            // effect is for would play at a third of its strength. What that
            // cost is a fade that runs backwards: the wood a tank drives into
            // ghosts as the hull arrives, and the same trunks then brighten to
            // full and start fading again from the top, which reads as the
            // effect restarting rather than as one tree leaving. <b>An alpha may
            // only fall here.</b> Whatever the ghost had got to by the time the
            // glacis reached the trunk is what it goes over at, and Gone takes
            // it from there to nothing - one clock and one direction. A trunk
            // felled before a hull was ever on its cell still goes over at full
            // strength, because nothing had ghosted it.
            if (tree.Going)
                continue;
            ...
            //
            // <b>And not a wood that is coming down.</b> A tree on a cell a
            // bulldozer has claimed is not ghosted at all: it is about to be
            // lying on the ground, and a thing that is leaving does not also
            // need to be seen through. Only the trees - the bushes and the
            // stones on that cell stay standing, so they go on ghosting for the
            // hull that is about to park on them.
            bool doomed = tree.Tier.Carries && razing is not null
                          && razing.Contains(tree.Cell);
            float want = tree.Fades && under && !doomed ? Ghost : 1.0f;
...
public sealed partial class PropNode : Node2D
{
    ...
    ///
    /// <b>And a third factor, on the same argument.</b> A felled trunk goes out
    /// by fading where it lies (<see cref="Gone"/>), and a shadow or a sole
    /// patch left at full strength under it would be the burnt bush's complaint
    /// word for word.
    /// </summary>
    public float Shown => Modulate.A * (1.0f - Spent) * (1.0f - Gone);

    /// <summary>
    /// How far this prop is through going over: 0 standing, 1 lying at the
    /// angle it settles at. Read off <see cref="Turn"/> rather than driving it -
    /// the pose is integrated (<see cref="Spin"/>) and this is the share of the
    /// arc it has covered, which is what the wind wants to know: a tree on its
    /// way down stops answering the gust.
    /// </summary>
    public float Down;

    /// <summary>
    /// How fast it is turning, in radians a second, signed the way
    /// <see cref="Turn"/> is.
    ///
    /// <b>The fall is integrated and not played, and that is the whole of what
    /// makes it read as a tree rather than an animation.</b> A clock from 0 to 1
    /// has to be shaped by hand and is the same shape every time; a trunk hinged
    /// at its root plate is a pendulum, <c>a'' = k*sin(a)</c>, and the shape
    /// falls out: it hangs at a few degrees while the hull leans on it, and the
    /// last third of the arc goes in a fraction of the time the first third
    /// took. It also takes a push - a heavy at speed throws a tree over harder
    /// than one crawling into it - which a clock cannot be told at all.
    /// </summary>
    public float Spin;

    /// <summary>Whether the crown has reached the ground at least once.
    /// A trunk goes on bouncing after it (<see cref="Grove.FallBounce"/>) and
    /// is not done until <see cref="Spin"/> is zero, so this is "has landed"
    /// rather than "is still".</summary>
    public bool Lying;

    /// <summary>How far through fading away it is once it has landed: 0 lying
    /// there, 1 gone. Separate from <see cref="Down"/> because the two are the
    /// two halves anybody watching names - it falls, and then it slowly is not
    /// there - and one clock doing both would have to fade while falling.
    /// </summary>
    public float Gone;

    /// <summary>Which way it goes over and how far, as a signed share of
    /// <see cref="Grove.FallAngle"/>: negative to the left, and a little more or
    /// less than one so a rank of trunks does not stop at one angle. Zero on a
    /// prop nobody has felled.</summary>
    public float Falls;

    /// <summary>
    /// How far over the prop has turned about its foot, in radians: zero
    /// upright, negative to the left.
    ///
    /// <b>A turn and not a lean, and that is the difference between falling and
    /// smearing.</b> The wind is a shear (<see cref="Lean"/>): every row of the
    /// art slides sideways by its height above the foot, the crown keeps the
    /// height it had, and the tree stays drawn the way it grew. That is exactly
    /// right for a gust and exactly wrong for a tree coming down - measured on
    /// the first cut of this effect, where a wood felled by shear read as
    /// melting sideways. A rotation takes the crown over with the trunk, which
    /// is what a felled tree does.
    ///
    /// <b>Kept beside the lean rather than folded into it.</b> They compose -
    /// the shear first, the turn about the foot after - so a trunk on its way
    /// down still answers the gust that is blowing, and neither has to know
    /// about the other.
    /// </summary>
    public float Turn;

    /// <summary>How much of its height it still has, for the two readers that
    /// project rather than draw: the cast shadow and the patch it keeps clear
    /// (<c>Stage3D.Cast</c>). They flatten the height column on to the ground,
    /// so what they want from the turn is how much height is left to
    /// throw.</summary>
    public float Squat => Mathf.Cos(Turn);

    /// <summary>Seconds left before it starts to go. The stagger across one
    /// cell, so eight trees are not one animation played eight times - see
    /// <see cref="Grove.Smoulder"/>, where the same hash does the same job for
    /// the fire.</summary>
    public double Waits;

    /// <summary>Whether it has been told to go over.</summary>
    public bool Going;
    ...
    /// </summary>
    public Transform2D Bend
    {
        get
        {
            var shear = new Transform2D(new Vector2(1.0f, 0.0f),
                                        new Vector2(-Lean, 1.0f), Vector2.Zero);
            // The shear first and the turn about the foot after - see Turn.
            // On the flat board the rotation is taken in screen space rather
            // than in the world's, which foreshortens height: a wood felled
            // there lies down a little flatter than the same wood does on the
            // stage. Named rather than corrected, for the reason the flat board
            // has no craters either - this effect is judged in 3D.
            return Turn == 0.0f
                ? shear : new Transform2D(Turn, Vector2.Zero) * shear;
        }
    }

    /// <remarks>
    /// <b>The two fades that are not in <see cref="CanvasItem.Modulate"/> are
    /// multiplied in here, and leaving them out was a real bug rather than a
    /// difference of boards.</b> <see cref="Shown"/> is the one statement of how
    /// much of a prop is on screen and it has three factors: the reveal's ghost,
    /// which is the node's own alpha and arrives for free; the burning-out; and
    /// the going-out of a felled trunk. The stage reads the product
    /// (<c>Stage3D.BuildTrees</c>) and the flat board read only the first, so a
    /// trunk here lay at full strength for the whole of <see cref="Grove.FadeFor"/>
    /// and then went in the frame <c>Gather</c> freed it - which is not a slow
    /// disappearance, it is a pop with a long wait in front of it.
    ///
    /// Multiplied into the draw rather than into <c>Modulate</c>, because that is
    /// the reveal's and has one writer.
    /// </remarks>
    public override void _Draw()
    {
        if (Lean != 0.0f || Turn != 0.0f)
            DrawSetTransformMatrix(Bend);
        DrawTextureRect(ShownArt,
                        new Rect2(-ShownFoot * Pixels, ShownSize * Pixels), false,
                        new Color(1.0f, 1.0f, 1.0f,
                                  (1.0f - Spent) * (1.0f - Gone)));
    }
}
