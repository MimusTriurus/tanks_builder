...
public sealed partial class TankSprite : Node2D
{
    ...
    /// <summary>
    /// Whether the gun sits in a ring of its own or in the hull.
    ///
    /// Copied off <see cref="MovementProfile.Turreted"/> when the vehicle is
    /// built - the sprite is handed the answer rather than the profile, because
    /// what it needs is one bool and the whole class table would be a second
    /// route to every other figure on it.
    ///
    /// <b>Why it lives on the sprite at all: the renderer cannot tell a casemate
    /// from a turret.</b> Every set has a turret layer and every turret layer is
    /// rendered spinning about the ring axis, which is the only thing a layer of
    /// that name can mean. So the destroyer's and the mortar's casemates arrive
    /// as twenty-four frames of a box rotating on the spot, and something has to
    /// refuse to show them that way.
    /// </summary>
    public bool Turreted = true;

    /// <summary>
    /// Where the gun points, in the same degrees as <see cref="HullFacing"/>.
    ///
    /// <b>A property rather than the field it used to be, and the reason is
    /// <see cref="Turreted"/>.</b> On a casemate this reads the hull back
    /// whatever anyone wrote, so the twenty-odd places that lay, spin, scan,
    /// stabilise or reset a turret need no case of their own: every one of them
    /// goes nowhere on a class that has no ring, and every reader - the bore, the
    /// flash, the fume, the fire gate, the panel - sees a gun welded to the hull.
    /// The alternative was those twenty-odd sites each asking whether this tank
    /// has a turret, which is twenty-odd chances for one of them not to.
    ///
    /// The backing value is kept rather than discarded, so a class flagged back
    /// to turreted at runtime resumes from where its ring was left instead of
    /// snapping to a heading nobody chose.
    /// </summary>
    public double TurretFacing
    {
        get => Turreted ? _turret : HullFacing;
        set => _turret = value;
    }

    private double _turret = 270.0;
    ...
    /// <summary>Which flash to draw. <b>Built - the game's track, decided
    /// 2026-09-07</b> (docs/effects-plan.md, "Дорожка по умолчанию"), the same
    /// decision as <see cref="ProcSmoke.OnByDefault"/>. Every set on disk
    /// carries a stamped bore, so nothing falls back; where one did not, Built
    /// would fall back to the sheet and Rendered to it too - see
    /// <see cref="ActiveSource"/>. The other two stay for the A/B, which is what
    /// keeps "the built one is better" a picture rather than a memory.</summary>
    public FlashSource Source = FlashSource.Built;
    ...
    /// <summary>Smoking without burning - the knocked-out hull. The column
    /// draws on either flag; the flame on <see cref="Burning"/> alone. See
    /// <see cref="Wreck.Disabled"/>.</summary>
    ...
    /// <summary>
    /// Which rung of the tube's ladder of laying angles it is on: 0 for the
    /// level gun, up to the atlas's last rendered elevation.
    ///
    /// <b>The second axis of one layer, and it idles at level for
    /// <see cref="RecoilPhase"/>'s reason</b> - a gun is always laid at
    /// something, and level is what it is laid at with nothing to shoot. The two
    /// are composed into the layer's phase by
    /// <see cref="AtlasSet.BarrelPhase"/>, which is the one place the renderer's
    /// <c>phase = elev·len(travel) + travel</c> is written on this side.
    ///
    /// Written by whoever is laying the gun - see <c>Main.UpdateAttack</c>,
    /// which already swings the ring or the hull and now lifts the tube in the
    /// same breath. Not settled here, because how high the tube stands is a fact
    /// about the shot and not about the drawing.
    /// </summary>
    public int BarrelRung;
    ...
    ///
    /// <b>Knocked out or destroyed, since 2026-09-08</b> - <see cref="Wreck.Out"/>,
    /// not <see cref="Wreck.Dead"/>. The pose is the knocked-out tank's picture
    /// (see <see cref="Wreck.Dim"/> for why); a destroyed one adds
    /// <see cref="Turretless"/>.
    ///
    ...
    public bool Wrecked
    {
        ...
        set
        {
            if (value == _wrecked)
                return;
            _wrecked = value;
            // A hull put back is a hull drawn whole: the fade below is a wreck's
            // and goes with the wreck, through the one flag a reset clears - and
            // so does the turret, back on its ring.
            if (!value)
            {
                Presence = 1.0;
                Turretless = false;
    ...
    /// <summary>
    /// Whether the turret has been blown off - the destroyed tank's picture,
    /// on top of <see cref="Wrecked"/>.
    ///
    /// <b>The rules have three states and the board needs three pictures.</b>
    /// Running, knocked out - the pose: gun dropped, belts slack, paint dimmed,
    /// a grey smoulder - and destroyed, which is a hull with no turret, black,
    /// burning, and then gone. Before this the destroyed tank owned the pose
    /// and the knocked-out one only a tint, and the two were not told apart at
    /// board zoom.
    ///
    /// <b>Set true launches the turret</b> (<see cref="TurretToss"/>): it leaves
    /// the ring on the frame of the detonation - the blast is the tank
    /// breaking, see <see cref="Wreck"/> - spins up and out, and lands beside
    /// the hull, where it chars with it and fades with it. Every turret layer
    /// (live, drooped, gun) stops drawing on this flag - see
    /// <c>EffectLayer.Wanted</c> - so there is one turret on screen at all
    /// times, and it is the one in the air.
    ///
    /// Cleared by <see cref="Wrecked"/> going false, which is what every reset
    /// does; not cleared by anything else, for <see cref="Wreck.Kill"/>'s
    /// reason - a hull does not grow its turret back.
    /// </summary>
    public bool Turretless
    {
        ...
        set
        {
            if (value == _turretless)
                return;
            // **A hull cannot lose a mount that was never on a ring.** Refused
            // here rather than gated at the one place that sets it, because
            // every writer has to be refused and there is no reading of this
            // flag that is right on a casemate: it takes the mount off the board
            // and launches it (see below), and a welded box does not come off.
            //
            // What a dead casemate looks like instead: the box stays, in the
            // hull layer it was rendered in, and chars and burns with the hull -
            // its gun drops to its stop on a layer of its own
            // (AtlasSet.WreckBarrelName). The picture that used to be drawn here
            // was a StuG's superstructure spinning into the air and an open
            // bathtub left behind, with a black gash where the generator builds
            // no deck under a mount.
            //
            // Asked of the atlas and not only of the class flag, because the
            // atlas is what has the layer: a set with no turret layer has no
            // turret to take off whatever any table says.
            if (value && (!Turreted || Atlas?.HasTurretLayer == false))
                return;
            _turretless = value;
            if (value)
                _toss?.Launch(HullFacing, TurretFacing);
            else
                _toss?.Reset();
            QueueRedraw();
        }
    ...
    private bool _turretless;

    /// <summary>
    /// Whether this wreck kept its turret on its ring - <see cref="Wreck.Seated"/>,
    /// which is where the argument is.
    ///
    /// Read by the mount's own layers through <c>EffectLayer.Mount</c> and by
    /// nothing else: a hull knocked out by a mine draws slack belts under a
    /// turret still pointing where it was, because that is what a charge under a
    /// track does. Cleared with <see cref="Wrecked"/>, like every other part of
    /// the pose.
    /// </summary>
    public bool TurretSeated
    {
        get => _turretSeated;
        set
        {
            if (value == _turretSeated)
                return;
            _turretSeated = value;
            QueueRedraw();
        }
    ...
    private bool _turretSeated;
    private TurretToss? _toss;

    /// <summary>The thrown turret, for the checks and the readout: where it is
    /// in its flight, or null on a set without one.</summary>
    public TurretToss? Toss => _toss;

    /// <summary>
    /// How much of the tank is drawn, 1 whole to 0 gone - <see cref="Wreck.Presence"/>.
    ///
    /// <b>A modulate, and an alpha-only one, which is why the note on
    /// <see cref="Char"/> does not apply.</b> That note refuses a modulate for
    /// the char because a per-channel multiply cannot blacken paint the way the
    /// shader does; this multiplies nothing but coverage, and coverage is exactly
    /// what leaving the board is. On the root, so every layer - hull, turret,
    /// belts, tile, the fire's stand-ins - goes together and the stage's render
    /// target sees one fading picture rather than parts.
    /// </summary>
    public double Presence
    {
        ...
        set
        {
            double now = Math.Clamp(value, 0.0, 1.0);
            if (now == _presence)
                return;
            _presence = now;
            Modulate = new Color(1.0f, 1.0f, 1.0f, (float)now);
            Visible = now > 0.0;
        }
    }

    private double _presence = 1.0;
    ...
    internal static EffectLayer MakeLayer(string layer) => layer switch
    {
        ...
        AtlasSet.WreckBarrelName => EffectLayer.WreckBarrel(layer),
    ...
    public void Repair()
    {
        ...
        _wear.Clear();
        Penetrations = 0;
        // The knocked-out dimming goes with the marks: nothing else clears it,
        // because the wreck's own path only runs on a hull that is out.
        Char = 0.0;
        Smouldering = false;
        QueueRedraw();
    }
    ...
    public override void _Ready()
    {
        ...
        // Last, above everything of this tank's own: a turret in the air is over
        // its fire and its smoke, and once down it sorts itself back into the
        // turret's slot. See TurretToss.
        AddChild(_toss = new TurretToss { Tank = this });
    }
    ...
    private sealed partial class SheetFlash : Node2D
    {
        ...
        public override void _Draw()
        {
            ...
            if (!_drawn)
                return;
            // Laid, like everything else that hangs off the mouth - but by a
            // screen angle rather than by a turn about the trunnion, because a
            // drawn sheet is a picture of a flame and not a model of one. See
            // AtlasSet.Tip, which is nought on a level gun and on any set that
            // shipped no mounting, so neither pays for this.
            Vector2 along = atlas!.GroundDirection(Tank.TurretFacing)
                                 .Rotated(atlas.Tip(Tank.TurretFacing, Tank.BarrelRung));
            Vector2 across = new Vector2(-along.Y, along.X).Normalized();
            Vector2 muzzle = atlas.Muzzle(Tank.TurretFacing, Tank.BarrelRung)
