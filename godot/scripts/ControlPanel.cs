...
public sealed partial class ControlPanel : PanelContainer
{
    ...
    {
        // Clipped with an ellipsis, not wrapped: a button's text is its own
        // minimum width, and one long title widens the list past the panel -
        // see the caption in Slide. The tooltip carries the whole of it.
                      ...
                      Func<string>? note = null)
    {
        ...
        text = Text.Title(id, text);
        // Wrapped, like every caption here - see Readout: a label that does not
        // wrap asks the list for its whole text as a minimum width, the list
        // grows past the panel's Width, and the scroll (horizontal disabled)
        // clips the tail of every row. One long note under one slider took the
        // right edge off the whole panel.
        ...
        if (note is not null)
        {
            aside = new Label { AutowrapMode = TextServer.AutowrapMode.WordSmart };
                       ...
                       Func<int> get, Action<int> set)
    {
        ...
        caption.AddThemeFontSizeOverride("font_size", 12);
        Host.AddChild(caption);
        // Clipped rather than sized to its longest option, for the caption's
        // reason: an option carrying a live figure must not set the panel's
        // width.
