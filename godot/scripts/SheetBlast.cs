...
public sealed partial class SheetBlast : Node3D
{
    ...
    /// <summary>
    /// Whether this burst went off in a wood - <see cref="Wet"/>'s third case,
    /// answered the same way and for the same reason.
    ///
    /// <b>What a round bursting in a stand of trees throws is not earth.</b> It
    /// is leaf, bark and splintered wood, and none of it comes out of the ground:
    /// the cell keeps its face, so there is no crater under this one either - see
    /// <see cref="Stage3D.Splinter"/>. So the swap is the same swap the pond got:
    /// the mass moves exactly as it moves, and the substance decides only the
    /// colour.
    ///
    /// Read in <see cref="Build"/>, so it is set before the node is built - which
    /// is what that third pool does.
    /// </summary>
    public bool Leaf;

    /// <summary>
    /// Set this burst up as one that went off in a wood: the ramp, and the shape
    /// the ramp on its own is not enough for.
    ///
    /// <b>Water needed only the colour and this needs more, which is not an
    /// inconsistency - it is a different event.</b> A shell into a pond throws a
    /// column, exactly as a shell into earth does, and the substance decides
    /// nothing but the ramp. A shell that goes off among the crowns is not the
    /// same event drawn in another colour: it never reaches the ground, there is
    /// no earth under it to throw, and what stands in its way is the wood itself.
    /// So the six numbers below, each of them a consequence of that one sentence.
    ///
    /// <b>Called before <see cref="Build"/></b>, like <see cref="Wet"/> and for the
    /// same reason: the ramp is baked into the material there.
    /// </summary>
    public void Timber()
    {
        Leaf = true;
        ...
        Life = TimberLife;
        Lances = TimberLances;
        LanceFast = TimberLanceFast;
        LanceThin = TimberLanceThin;
    }

    /// <summary>Where a wood burst starts, in tile widths above the ground. Well
    /// over twice the earth burst's, because it is not a ground burst - a tree on
    /// this board stands about half a tile high, and the round met the crowns, not
    /// the floor. Not at the top of them either: seated at the crown line the mass
    /// stands clear above the wood and reads as a bonfire behind it, so the seat is
    /// inside the canopy and the trees are drawn through it.</summary>
    public const float TimberSeat = 0.24f;

    /// <summary>And how far it rises from there. A third of the earth burst's: a
    /// column stands up because there is a mass of thrown soil under it pushing;
    /// here there is nothing under it but more wood, and what the mass does is get
    /// caught.</summary>
    public const float TimberClimb = 0.16f;

    /// <summary>So it goes sideways instead - most of the way to a disc. The one
    /// number that decides whether this reads as a burst in a wood or as a bonfire
    /// behind it.</summary>
    public const float TimberFlat = 0.86f;

    /// <summary>Half again as wide as the earth burst. A round in the open throws
    /// from a point; a round in a stand of trees is met by the whole cell.
    /// </summary>
    public const float TimberReach = 0.34f;

    /// <summary>And it is over sooner. Most of an earth burst's two seconds is the
    /// column coming back down; nothing here was lifted far enough to have to fall.
    /// </summary>
    public const float TimberLife = 1.45f;

    /// <summary>
    /// The splinters, and this is the finding of the whole effect: they were
    /// already drawn.
    /// </summary>
    /// <remarks>
    /// The plan asked for splinters as though they were art nobody had made. They
    /// are <see cref="Lances"/> - the fast, thin, stretched puffs written to make an
    /// earth burst read as thrown clods rather than as smoke, and whose own note
    /// describes them as "radial spikes, each tapered, each overshooting the body
    /// it came out of". That is a photograph of shattered wood as much as it is one
    /// of thrown soil. So more of them, thinner and faster: earth is thrown in
    /// lumps, wood comes apart in slivers.
    /// </remarks>
    public const int TimberLances = 34;

    public const float TimberLanceFast = 2.60f;

    public const float TimberLanceThin = 0.30f;
    ...
    /// <summary>The haze pass: the same instance array, drawn under the cloud
    /// through <see cref="Hazing"/>. A second instance rather than a second model
    /// - one MultiMesh, written once a frame, read twice.</summary>
    private MultiMeshInstance3D? _haze;
    ...
    internal static Texture2D? SheetArt;
    internal static Texture2D? BulgeArt;
    internal static Texture2D? DentArt;
    ...
    public void Build(float tile, float squash, float rise)
    {
        ...
        _ink.SetShaderParameter("sheet", Art("puff_sheet.png", ref SheetArt));
        _ink.SetShaderParameter("bulge", Art("puff_bulge.png", ref BulgeArt));
        _ink.SetShaderParameter("dent", Art("puff_dent.png", ref DentArt));
        _ink.SetShaderParameter("smoke_ramp", Ramp(Smokes(Wet, Leaf), 256));
        ...
        // <b>And a wood burst's flash has to be let up to where the wood burst
        // is.</b> flame_low's own note calls it "how far above the seat the fire
        // survives", and for a burst seated a tenth of a tile off the ground that
        // is near enough true measured from the node's origin, which is what the
        // shader does. Timber seats the mass up in the crowns, and there the two
        // stop being the same question: at the written figure the whole cloud is
        // above the cut and there is no flash on the frame the round goes off.
        // Added to the whole ball rather than to the seat alone, and that is the
        // second half of the same difference: an earth burst is a column standing
        // on its fireball, so a cut just above the ground keeps the fire where the
        // reference has it and takes it off the two tile-widths of dust over it. A
        // wood burst has no column - the ball is the event - and a cut inside it
        // leaves the flash down among the trunks where the crowns cover it, which
        // on the board is no flash at all.
        //
        // Added rather than replaced, so the sentence in the shader stays the
        // sentence, and read out of the text rather than restated so that turning
        // one turns the other. The smoke screen paid for this lesson once already -
        // see ProcScreen.FlashReach.
        if (Leaf)
            _ink.SetShaderParameter("flame_low",
                ProcBlast.Uniform(Code, "flame_low")
                + (TimberSeat + TimberReach) * _tile);

        _fog = new ShaderMaterial { Shader = Hazing, RenderPriority = Stage3D.StandOrder - 1 };
        _fog.SetShaderParameter("sheet", Art("puff_sheet.png", ref SheetArt));
        _fog.SetShaderParameter("bulge", Art("puff_bulge.png", ref BulgeArt));
        _fog.SetShaderParameter("dent", Art("puff_dent.png", ref DentArt));
        _fog.SetShaderParameter("seat_y", 0.0f);
        ...
        AddChild(_cloud);
        _haze = new MultiMeshInstance3D
        {
            Multimesh = _many,
            MaterialOverride = _fog,
            SortingUseAabbCenter = false,
            Visible = false,
            Position = _cloud.Position,
        };
        AddChild(_haze);
        Dress();
    }
    ...
    private void Stand()
    {
        ...
        _fog?.SetShaderParameter("seat_y", Transform.Origin.Y);
        ...
        if (_haze is not null)
            _haze.Position = _nudge / _might;
    }
    ...
    public void Tick(double delta)
    {
        ...
        _fog?.SetShaderParameter("time", _clock);
        Dress();
    }
    ...
    {
        _live[uniform] = value;
        bool counted = Code.Contains("uniform int " + uniform + " = ",
                                     StringComparison.Ordinal);
        Variant sent = counted ? Mathf.RoundToInt(value) : value;
        // Both passes: the sheet's own numbers are shared between them, and a
        // uniform a shader does not declare is a parameter it never reads.
        _ink?.SetShaderParameter(uniform, sent);
        _fog?.SetShaderParameter(uniform, sent);
    }
    ...
    /// </summary>
    /// <remarks>Internal rather than private since 2026-09-09: the smoke screen
    /// draws the same three files (<see cref="ProcScreen"/>), and a second loader
    /// would be a second cache of 48MB.</remarks>
    internal static Texture2D Art(string file, ref Texture2D? kept)
    {
        if (kept is not null)
            return kept;
    ...
    /// </summary>
    internal static ImageTexture Ramp((float At, Color Ink)[] stops, int wide)
    {
    ...
    /// <summary>
    /// The same ramp for a burst in a wood: litter, and it runs the earth's way
    /// rather than the water's.
    ///
    /// <b>Dark leaf, olive litter, and the pale inside of a broken trunk.</b> The
    /// dark stop is canopy shadow, which is darker than any earth on this board -
    /// a wood is the one place a burst has something over it. The middle is the
    /// mulch of the floor thrown up with it. The top is what makes the picture
    /// read as wood rather than as dirty smoke: fresh splinters are nearly white
    /// and much paler than the bark they came off, so the brightest of the mass is
    /// the newest wound in it.
    ///
    /// <b>And it climbs, like the water's and unlike the earth's, but not as
    /// far.</b> Earth darkens as it thickens because earth absorbs; a cloud of
    /// shredded wood has bright chips through it that catch light at every depth.
    /// Taken to white it would be a splash, so the top stop stays warm and short
    /// of one.
    /// </summary>
    internal static readonly (float, Color)[] LitterStops =
    {
        (0.00f, new Color(0.115f, 0.125f, 0.085f)),
        (0.50f, new Color(0.400f, 0.375f, 0.235f)),
        (1.00f, new Color(0.880f, 0.845f, 0.660f)),
    };

    /// <summary>Which of the three the burst is made of - the one consequence of
    /// <see cref="Wet"/> and <see cref="Leaf"/>, as a method so a check can ask for
    /// it. Wet first, because a wood cannot stand in water and a caller that says
    /// both has asked the board a question with no answer.</summary>
    internal static (float, Color)[] Smokes(bool wet, bool leaf = false) =>
        wet ? FoamStops : leaf ? LitterStops : SmokeStops;
    ...
    internal static readonly (float, Color)[] FlameStops =
    {
    ...
    /// does.</summary>
    internal static readonly (float, Color)[] FadeStops =
    {
    ...
    /// <summary>
    /// The part both passes over the cloud are: which cell of the sheet a puff is
    /// on, where a corner lands inside it, and how far above the ground it is.
    ///
    /// <b>Shared rather than copied, because the haze has to land on the smoke to
    /// the pixel.</b> The second pass reads the same flipbook through the same
    /// turn at the same instant; a vertex stage written twice would drift the
    /// moment either copy is tuned, and a haze half a frame off its own smoke is
    /// a haze round nothing. Pasted into both shaders at
    /// <c>PUFF_FRAME</c>, the arrangement <see cref="ProcWave"/> already runs -
    /// each shader still declares the uniforms it uses, which is what a shader
    /// must do anyway.
    /// </summary>
    private const string PuffFrame = @"
varying float high;
varying vec4 mine;

// Where this corner sits in the cell it is on and in the cell after it, and how
// far between the two the puff has got. Two UVs rather than one because the turn
// is per-corner and the cell is per-instance, so both are settled in the vertex
// and the fragment is left with one mix.
varying vec2 pad;
varying vec2 pad_next;
varying float crossing;

// One cell of the sheet, as a corner's place inside it.
vec2 cell_of(vec2 inside, float f) {
    return inside / vec2(float(across), float(down))
...
void vertex() {
    ...
    // <b>The pass's own size, and the reason the haze is a pass rather than a
    // tint.</b> Hot air is a bigger thing than the smoke inside it, and a shimmer
    // drawn on exactly the smoke's silhouette is a shimmer the smoke covers -
    // which is what the first build of this was: measurably there, visibly
    // nothing. The reference video swells its distortion quad past its explosion
    // for the same reason. The cloud's own copy is a const at one, so this costs
    // it nothing.
    VERTEX.xy *= swell;

    // Which cell of the sheet this puff is on, from its own age - eased, so the
    // opening is quick and the dissolve is long.
    float frames = float(across * down);
    float head = pow(clamp(mine.z, 0.0, 1.0), max(sheet_ease, 0.05));
    float walk = head * (frames - 1.0);
    float f = floor(walk);
    float over = walk - f;
    if (still > -0.01) {
        f = floor(still);
    ...
    // The last cell has nothing after it, so it crosses to itself and the mix
    // below is a no-op there rather than a wrap round to the first frame.
    float f_next = min(f + 1.0, frames - 1.0);
    crossing = over * clamp(frame_blend, 0.0, 1.0);
...
";
    ...
    private static readonly string PuffCode = PuffBody.Replace("PUFF_FRAME", PuffFrame);

    /// <summary>The puff shader itself. Internal since 2026-09-09: the smoke
    /// screen is this same sheet with another placement and another life, and the
    /// rule this project runs on is one answer to one question - see
    /// <see cref="ProcBlast.DustInk"/>.</summary>
    internal static readonly Shader Puffing = new() { Code = PuffCode };

    private static readonly string HazeCode = HazeBody.Replace("PUFF_FRAME", PuffFrame);

    private static readonly Shader Hazing = new() { Code = HazeCode };

    /// <summary>The shader's text, for the panel and the check to read the
    /// numbers out of - <see cref="ProcBlast.FrameCode"/>'s arrangement. Both
    /// passes, because both have dials on them and the panel reads its defaults
    /// out of this.</summary>
    internal static string Code => PuffCode + HazeCode;

    /// <summary>
    /// The air the burst heats, as a second pass over the same puffs.
    ///
    /// <b>The reference video's second shader, with the one thing it needs already
    /// in the pack.</b> It bends the screen behind its explosion by the flipbook's
    /// optical-flow map, because that is the only per-pixel direction it has. This
    /// sheet ships something better and for this purpose more nearly right: a
    /// signed normal on every frame. Hot air bends light along the gradient of its
    /// own density, and the normal map <b>is</b> that gradient - the same texture
    /// the smoke is lit by, read for where it points rather than for how much it
    /// faces the sun. So no new art, no flow map to bake, and the haze lands on the
    /// smoke to the pixel because it is drawn from the same instance array through
    /// the same vertex stage.
    ///
    /// <b>Drawn under the cloud and not over it.</b> Priority nought against the
    /// cloud's one: the shimmer is between the ground and the smoke, so what it
    /// bends is the board, and the smoke then covers it wherever the smoke is
    /// thick. Over the top it would bend the cloud's own silhouette, which is a
    /// smear rather than a shimmer.
    ///
    /// <b>And it goes out early.</b> Heat haze is the fireball's, not the dust's -
    /// a plume of cold smoke standing on the board does not bend anything - so it
    /// is faded on the event's clock like the flame is, and for the same reason
    /// (see <c>flame_out</c>).
    /// </summary>
    private const string HazeBody = @"
shader_type spatial;
...
// Mipmapped, because the blur below is a mip level and nothing else. A separate
// blur pass over the screen would be a second render target for one quad.
...
uniform float sheet_ease = 0.78;
uniform float spin = 1.0;
uniform float seat_y = 0.0;
uniform float time = 0.0;

// How far the screen is pulled, in screen fractions. Small: UVs are normalised,
// so a tenth is the whole board and a hundredth is a plain shimmer.
// How much wider the haze is drawn than the smoke it belongs to. The one
// number without which none of the rest is visible.
uniform float swell = 1.55;
uniform float haze_warp = 0.013;
// How far up the mip chain a fully hazed pixel reads. The video's trick, which
// is the half of heat haze that is not displacement: air that shimmers also
// softens what is behind it, and a bend with no softening reads as a wobbling
// decal.
uniform float haze_blur = 1.3;
// How much of the bent screen is drawn over the unbent one.
uniform float haze_ink = 0.85;
// When the haze is out, in seconds of the event - flame_out's clock and
// flame_out's reason, a little longer because air stays hot after it stops
// glowing.
uniform float haze_out = 0.38;

PUFF_FRAME

void fragment() {
    vec4 puff = texture(sheet, pad);
    vec3 up = texture(bulge, pad).rgb;
    vec3 down_n = texture(dent, pad).rgb;
    if (frame_blend > 0.0) {
        puff = mix(puff, texture(sheet, pad_next), crossing);
        up = mix(up, texture(bulge, pad_next).rgb, crossing);
        down_n = mix(down_n, texture(dent, pad_next).rgb, crossing);
    }

    // The density gradient, which is the normal map read for its direction.
    vec3 face = normalize(up - down_n + vec3(0.0, 0.0, 1e-4));

    float hot = 1.0 - smoothstep(0.0, max(haze_out, 1e-3), time);
    float mask = clamp(puff.a * mine.w, 0.0, 1.0) * hot;
    if (mask <= 0.001) {
        ALBEDO = vec3(0.0);
        ALPHA = 0.0;
    } else {
        vec2 shift = face.xy * haze_warp * mask;
        ALBEDO = textureLod(screen, SCREEN_UV + shift, haze_blur * mask).rgb;
        ALPHA = mask * haze_ink;
    }
}
";
...
// <b>How much of the way to the next cell is drawn as the next cell</b> - the
// reference video's headline trick, kept, measured, and left switched off.
//
// The trick is sound and the arithmetic here is its: this cell and the next are
// both sampled and mixed by how far between them the puff is, sheet and both
// normal halves alike, so the flipbook crosses rather than steps. On the video's
// explosion it is the difference between a slideshow and an animation.
//
// <b>On this one it is worth nothing, and the measurement is why it defaults to
// nought.</b> Captures at eleven ages across the event, blended against stepped:
// the largest single pixel differed by 29 of 255 and fewer than five hundredths
// of one per cent of pixels differed by more than 8. Two reasons, and both are
// facts about this burst rather than about the technique. The sheet is
// sixty-four frames where the video's is twenty-five, so one step of it is a
// third of the change; and a puff is drawn about fifty pixels wide out of a
// 256-pixel cell, so what the step moves is a fraction of a screen pixel. On top
// of that the cloud is twenty-nine puffs with staggered births, so they do not
// step together and there is no moment when the picture jumps.
//
// Kept as a dial rather than deleted because it is three texture samples away at
// any time, and the next sheet - a shorter one, or one drawn bigger - is the
// case it was written for. See docs/blast.md.
uniform float frame_blend = 0.0;

// <b>How the sheet's own clock runs against the puff's.</b> The playhead raised
// to this power: below one the sheet spends its early cells quickly and its late
// ones slowly, which is what a burst does - it opens in a few frames and then
// stands there dissolving for the rest of the event. Straight linear playback
// gives the opening and the dissolve the same number of frames each, so the
// opening is over before it is seen and the dissolve is a sequence of visibly
// separate pictures. One at nought and it is off.
uniform float sheet_ease = 0.78;
...
// The cloud is drawn at the size the model asks for - see swell in the haze.
const float swell = 1.0;
...
void fragment() {
    ...
    vec3 up = texture(bulge, pad).rgb;
    vec3 down_n = texture(dent, pad).rgb;
    // <b>The normals cross with the sheet or the light stutters on its own.</b>
    // Mixing the colour and stepping the normal is worse than stepping both: the
    // silhouette then slides while its shading jumps, which reads as the cloud
    // being relit every frame. The reference video makes the same correction and
    // for the same reason.
    // Branched on the uniform and not on the crossing, so the whole draw takes
    // one side of it: three samples when the sheet steps, six when it crosses.
    if (frame_blend > 0.0) {
        puff = mix(puff, texture(sheet, pad_next), crossing);
        up = mix(up, texture(bulge, pad_next).rgb, crossing);
        down_n = mix(down_n, texture(dent, pad_next).rgb, crossing);
