...
public sealed partial class ProcSlam : Node3D
{
    ...
    /// it fades out over below that - the burst frame's own <c>root</c> pair and its
    ...
    /// <summary>
    /// Which rung of <c>Stage3D</c>'s ladder this burst sorts on.
    ///
    /// <b>Depth is not enough when the burst and the tank are at the same
    /// point.</b> Both halves of this effect sit at <c>Stage3D.StandOrder</c>,
    /// the tanks' own rung, and inside a rung the camera sorts transparent
    /// surfaces by each object's origin - one answer for a whole quad, because a
    /// billboard hull has no depth of its own to test against. That is right for
    /// every event this draws but one: a round arrives on a plate, a shell bursts
    /// on a wall, and both of those are measurably outside the hull. A charge
    /// under the belly is <em>inside</em> it, so the sort is a coin toss and both
    /// faces of the coin are wrong - a hair nearer the camera paints the burst
    /// over the whole tank, a hair further hides it altogether.
    ///
    /// So a mine drops a rung (<c>Stage3D.DressOrder</c>), which is the same move
    /// and the same argument as a bush a tank has parked on: give up the sort
    /// against a hull and let the hull win by priority - see
    /// <c>Stage3D.DressOrder</c> and <see cref="PropTier.UnderTanks"/>. What is
    /// then seen of the blast is what got out past the silhouette, which is what
    /// a confined charge shows anyway.
    ///
    /// Written on every throw, because the bursts come out of a pool: one dropped
    /// a rung for a mine and handed back would put the next shell's burst under
    /// the tank it hit.
    /// </summary>
    public int Order
    {
        // Off the material rather than off the field, for Placed's reason: the
        // write is the whole subject and nothing about a picture says whether it
        // landed. The field is the answer only before Build, when there is no
        // material to ask.
    ...
    /// <summary>
    /// The burning half's own rung, which is <see cref="Order"/>'s unless somebody
    /// says otherwise.
    ///
    /// <b>Two quads, so two answers, and the mine is what needs them.</b> A charge
    /// goes off under the leading end of a hull: for a tank driving at the camera
    /// that point is in <em>front</em> of the hull's contact line, and light in
    /// front of a tank belongs in front of it - drawn behind, the flash is a lamp
    /// somebody put on the far side of the tank. The dust is the other way round
    /// and has a report behind it: a cloud laid over the hull is what "дым
    /// просвечивается сквозь танк" was about, and it was dust every time.
    ///
    /// So the fire follows the geometry - <c>Stage3D.Mine</c> hands it the side the
    /// charge is on - and the dust stays under the tank whichever side that is.
    /// Written on every throw with <see cref="Order"/>, for the pool's reason.
    /// </summary>
    public int Lit
    {
        get => _fireInk?.RenderPriority ?? _lit;
        set
        {
            _lit = value;
            if (_fireInk is not null)
                _fireInk.RenderPriority = value;
        }
    }

    private int _lit = Stage3D.StandOrder;
    ...
    /// <see cref="SheetBlast.Fire"/>'s reason: the organ that fires it is a key on a
    /// bench.</summary>
    public void Fire() => _clock = 0.0f;

    /// <summary>Whether the clock stands still - <see cref="SheetBlast.Hold"/>, and
