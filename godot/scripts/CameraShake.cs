...
public sealed class CameraShake
{
    ...
    /// <summary>
    /// The ground shaking under a detonation - the second thing this camera
    /// does, and it is not the spring.
    ///
    /// <b>The spring is a jolt, and a jolt is not what a tank going up feels
    /// like.</b> Measured on the events bench: the death kick through
    /// <see cref="Fire"/> peaks at 6.5 world px on the frame after the blast and
    /// is under a pixel eight frames later - one dip, up and back, in an eighth
    /// of a second, straight up the screen. Correct for a gun, which is one
    /// impulse; invisible for a detonation, which was asked about as "is the
    /// shake even on". A blast is a shockwave and then the ground settling, and
    /// the view has to <em>tremble</em>: both axes, several cycles, dying out
    /// over half a second. So this is a second term added to the spring's, a
    /// decaying sum of sines - two frequencies per axis so it does not read as
    /// a pendulum, phases offset so the axes do not move in step - and it is
    /// deterministic, which <c>--capture</c> requires and a noise field would
    /// not give.
    ///
    /// The envelope is (1 - t/T)^2: hard at the start, gone at <see cref="BlastSeconds"/>
    /// exactly rather than trailing off below a pixel for another second, which
    /// the snap to whole pixels would otherwise turn into a late one-pixel twitch.
    /// </summary>
    public const double BlastSeconds = 0.55;

    private double _rumbleAmp;
    private double _rumbleAge = double.PositiveInfinity;

    /// <summary>A detonation wanting a tremble of <paramref name="pixels"/>
    /// peak at <see cref="Level"/> 1. A second blast inside the first one's
    /// half-second restarts the clock and takes the larger amplitude: two tanks
    /// going up together is one bigger shake, not two overlaid ones, which would
    /// beat against each other.</summary>
    public void Blast(double pixels)
    {
        _rumbleAmp = Math.Max(pixels * Level, Rumbling ? _rumbleAmp : 0.0);
        _rumbleAge = 0.0;
    }

    public bool Rumbling => _rumbleAge < BlastSeconds;

    /// <summary>The rumble's displacement now, world units, unsnapped.</summary>
    public Vector2 Rumble
    {
        get
        {
            if (!Rumbling)
                return Vector2.Zero;
            double t = _rumbleAge;
            double env = 1.0 - t / BlastSeconds;
            env *= env;
            double x = 0.75 * Math.Sin(2.0 * Math.PI * 11.0 * t)
                       + 0.35 * Math.Sin(2.0 * Math.PI * 17.0 * t + 1.3);
            double y = 0.90 * Math.Sin(2.0 * Math.PI * 9.0 * t + 0.7)
                       + 0.30 * Math.Sin(2.0 * Math.PI * 23.0 * t);
            return new Vector2((float)(_rumbleAmp * env * x), (float)(_rumbleAmp * env * y));
        }
    }

    /// <summary>Where the view is displaced to, in world units, unsnapped: the
    /// spring's jolt plus the blast's rumble. The snapping belongs with the
    /// zoom - see <see cref="ScreenOffset"/>.</summary>
    public Vector2 Offset => _offset + Rumble;
    ...
    public Vector2 ScreenOffset(float zoom)
    {
        if (zoom <= 0.0f)
            return Vector2.Zero;
        Vector2 all = Offset;
        return new Vector2(Mathf.Round(all.X * zoom) / zoom,
                           Mathf.Round(all.Y * zoom) / zoom);
    }

    public bool Moving => _offset.Length() > 1e-4f || _rate.Length() > 1e-3f || Rumbling;
    ...
    public void Reset()
    {
        ...
        _rate = Vector2.Zero;
        _rumbleAmp = 0.0;
        _rumbleAge = double.PositiveInfinity;
    }
}
