...
public sealed partial class BoardMap
{
    ...
    /// <summary>Low ground with water standing in it - a ford's depth, the
    /// surface well below the bank.</summary>
    public const char Wet = 'w';

    /// <summary>
    /// Low ground filled to the brim: deep water, its surface level with the
    /// bank at the datum.
    ///
    /// <b>Depth is the surface's height, not a second kind of liquid.</b> The
    /// game's water rises and falls, and what it does is fill the hollow further
    /// or drain it - so a hollow with water in it is <see cref="Wet"/> at a
    /// ford's depth and this at a level's, and nothing else about the cell
    /// changes. The floor names <see cref="Foundation.Deep"/> so that the rules
    /// (nothing drives in) and the picture (the surface stands a whole level
    /// over the bed) read one slot; the flood mask is set too, because the
    /// surface mesh and the swell index by it.
    /// </summary>
    public const char DeepWet = 'd';
    ...
    /// <summary>Where the five are parked: one row, every other column. The same
    ...
    /// the line is for.</b> Abreast at one height is how the class sizes are
    /// judged - see MovementProfile.Size - and with the light on (11,3) the line
    /// is missing the smallest of the five. It is there because a hill with six
    /// approaches is worth having a tank on: the lean, the standing height and the
    /// six ramps meeting the top are all things a tank on the summit says at a
    /// glance and an empty summit does not. R puts it back there rather than into
    /// the line, which is what makes it a home and not a start.
    ///
    /// <b>The destroyer and the mortar went on the same row rather than anywhere
    /// more interesting</b>, and the medium's neighbour is the destroyer on
    /// purpose: those two are one class in every driving figure and differ only in
    /// where the gun sits, so the pair standing side by side is the whole of that
    /// comparison. Columns 8 and 10 rather than 11 and 13 because (11,2) is one of
    /// the rosette's ramps and a home on a slope tilts the hull it is meant to be
    /// judged against.
    ///
    /// <b>The order of this list is the order tanks claim it in</b> - see
    /// Parking.Pair - so the rosette staying first is what keeps the light on the
    /// hill and everything else where it was.</summary>
    public static readonly Parking[] BenchHomes =
    {
        new Vector2I(11, 3), new Vector2I(4, BenchHomeRow),
        new Vector2I(6, BenchHomeRow), new Vector2I(8, BenchHomeRow),
        new Vector2I(10, BenchHomeRow),
    };
