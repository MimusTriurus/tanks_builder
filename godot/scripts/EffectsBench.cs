...
/// alone:
///
///     Godot_v4.7.2-stable_mono_win64.exe --path godot res://Effects.tscn
///
...
public sealed partial class EffectsBench : SceneRoot
{
    ...
    private void ReadFlags()
    {
        ...
        for (int i = 0; i < args.Length; i++)
        {
            ...
            if (args[i] == "--fire")
                _shoot = true;
            // The blast wave on the aimed cell, for the rule --fire exists under.
            else if (args[i] == "--wave")
                _surge = true;
            // The drawn explosion on the aimed cell, same rule.
            else if (args[i] == "--toon")
                _toon = true;
            // The fireball on the aimed cell, same rule.
            else if (args[i] == "--ball")
                _ball = true;
            // A smoke screen on the aimed cell, same rule - and the one flag here
            // that leaves something standing when the frame is taken, which is
            // what --capture-at names an age of.
            else if (args[i] == "--screen")
                _veilNow = true;
            // And taken off again that many seconds in, so the clearing has a
            // frame of its own without a hand on the keyboard - a screen is the
            // one effect here whose end is a second event rather than the end of
            // the first.
            else if (args[i] == "--screen-off" && i + 1 < args.Length
                     && float.TryParse(args[i + 1], NumberStyles.Float,
                                       CultureInfo.InvariantCulture, out float off))
            {
                _veilNow = true;
                _veilOffAt = Mathf.Max(off, 0.0f);
                i++;
            }
            // Which of the two ends that is: a hull through it rather than the
            // round running out.
            else if (args[i] == "--screen-gust")
                _veilGustOff = true;
    ...
    public override void _Process(double delta)
    {
        ...
        // The screen's own second event, if a flag asked for one - see
        // EffectsBench.Screen.
        ScreenTick(delta);
    ...
    private string Note()
    {
        ...
        return $"board {_map.Columns}x{_map.Rows}, wood, pond and a ring of brick, "
               ...
               + "S lays a smoke screen, D takes it off (shift: a hull through),\n"
    ...
    public override void _UnhandledInput(InputEvent @event)
    {
        ...
        switch (key.Keycode)
        {
            ...
            case Key.S:
                LayScreen();
                break;
            // The two ends of a screen's life on one key: shifted, a hull went
            // through it. Two presses rather than two keys, because the second is
            // the first at another speed - ProcScreen.Lift's own argument.
            case Key.D:
                LiftScreen(key.ShiftPressed);
                break;
            case Key.W:
                Surge();
                break;
