...
{
    /// <summary>Which rung of Stage3D's ladder a crater sorts on: the tanks'
    /// own, and it cannot leave. Named rather than left inside Build so the pair
    /// with <see cref="Deep"/> can be asserted without a board.
    ///
    /// <b>A crater is ground and belongs down with the belt marks, and it will not
    /// go.</b> Tried at <c>RutOrder</c>, at <c>ShadowOrder</c> and at
    /// <c>DressOrder</c>: on every one of the three the mark stops drawing at
    /// all - not covered by something, simply absent, on open ground as much as
    /// under a hull. What swallows it below this rung I have not found; the
    /// contact shadow draws happily at <c>ShadowOrder</c>, so it is not that the
    /// rung is unreachable. Written down because the next person will otherwise
    /// spend the same hour: the ladder is not the tool for this one.</summary>
    public const int Rung = Stage3D.StandOrder;

    /// <summary>
    /// How far back the mark sorts inside that rung, in world units.
    ///
    /// <b>This is what keeps a crater under the tank standing in it</b>, and it is
    /// the fix for "воронка отображается поверх танка". Inside a rung the camera
    /// sorts transparent surfaces by each object's own origin, so a hole and the
    /// hull over it - which are at the same point, give or take the charge's
    /// offset - settled it by a coin toss, and the coin came up over the tank.
    /// <c>SortingOffset</c> moves the depth a surface is sorted at without moving
    /// the surface, so the mark keeps its rung and goes to the back of it.
    ///
    /// <b>Far enough back to lose to everything, on purpose.</b> A board is a few
    /// thousand world units across and a crater should be under anything standing
    /// on it, wherever that thing stands - not just under the tank whose mine dug
    /// it. So this is not tuned to a hull's length; it is larger than the board.
    /// </summary>
    public const float Deep = 1000.0f;
    ...
    {
        _ink = new ShaderMaterial { Shader = Etching, RenderPriority = Rung };
