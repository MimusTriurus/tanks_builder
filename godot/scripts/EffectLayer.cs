...
public sealed partial class EffectLayer : Node2D
{
    ...
    public enum Life
    {
        ...
        /// <summary>The wrecked version of it - since 2026-09-08 the knocked-out
        /// tank's pose as much as the destroyed one's; see
        /// <see cref="TankSprite.Wrecked"/>.</summary>
        Dead,
    }
    ...
    /// <remarks>
    /// The pair as declared, which for the mount is the turreted case. Which
    /// layer actually stands in *in a given set* is asked of the set instead -
    /// see <see cref="AtlasSet.WreckMount"/> and <see cref="Mount"/>: a casemate
    /// has no turret half at all and its gun's dead twin is
    /// <c>wreck_barrel</c>.
    /// </remarks>
    public string StandIn = "";

    /// <summary>
    /// Whether this layer is part of the turret and its gun.
    ///
    /// <b>Set so a wreck can be wrecked in parts.</b> What knocked a tank out
    /// decides which of its parts are in the wreck's pose: a round through the
    /// armour drops the gun and slackens the belts, a mine breaks the running
    /// gear and leaves the mount alone - <see cref="Wreck.Seated"/>. The belts
    /// and the mount are already separate live/dead pairs, so the distinction
    /// costs one flag and no new art.
    /// </summary>
    public bool Mount;
    ...
    /// <summary>The same question asked of a hull whose turret may have been left
    /// on its ring - <see cref="Mount"/> and <see cref="Wreck.Seated"/>. Its own
    /// method rather than a second argument to <see cref="LiveNow"/>, because the
    /// pair reads better as "the wreck's answer, and the part's answer to
    /// it".</summary>
    public bool Standing(bool wrecked, bool seated, bool standInReady) =>
        LiveNow(wrecked && !(Mount && seated), standInReady);
...
",
    ...
    /// <summary>
    /// The gun dropped to its stop on a dead casemate, and nothing else.
    ///
    /// <see cref="WreckTurret"/>'s twin for a vehicle whose mount is welded on.
    /// There the tube is inside the mount layer because the mount fell and took
    /// the gun with it; here the box did not move - it is in the hull layer, and
    /// a dead casemate keeps it, chars with it and burns with it. So this layer
    /// stands in for the live <see cref="Barrel"/> alone.
    ///
    /// <c>Clocks.Body</c> rather than <c>Clocks.Recoil</c>, for the reason the
    /// wrecked turret has it: a dead gun does not recoil, so the layer has one
    /// phase and no stroke to walk.
    /// </summary>
    public static EffectLayer WreckBarrel(string layer) => new()
    {
        Layer = layer,
        FollowsHull = false,
        Clock = Clocks.Body,
    ...
    public static EffectLayer Turret(string layer) => new()
    {
        ...
        Mount = true,                 // and a mine leaves it standing - Wreck.Seated
    };
    ...
    public static EffectLayer Barrel(string layer) => new()
    {
        ...
        When = Life.Alive,
        StandIn = AtlasSet.WreckTurretName,
        // and it is part of the mount, so it stays up with it - Wreck.Seated
        Mount = true,
    };
    ...
    private int Wanted
    {
        get
        {
            ...
            // Which layer stands in, resolved against the set rather than read
            // off the field: every part of the mount pairs with "the wreck's
            // mount layer", and which one that is depends on the vehicle - the
            // whole mount on a turreted tank, the drooped gun on a casemate,
            // whose box never leaves the hull layer. Naming `wreck_turret` here
            // would leave a casemate's live gun drawing on a dead tank beside
            // its own drooped twin: two guns, which reads as a broken atlas.
            string standIn = Mount ? Tank.Atlas.WreckMount : StandIn;
            if (!Standing(Tank.Wrecked, Tank.TurretSeated,
                          standIn != "" && Tank.Atlas.Has(standIn)))
                return -1;
            return Clock switch
            {
                ...
                // a broken switch.
                // Or smouldering: a knocked-out hull smokes without a flame -
                // see TankSprite.Smouldering.
                Clocks.BurningSmoke => (Tank.Burning || Tank.Smouldering)
                                       && !Tank.SmokeIsProcedural
                    ? Tank.BurnPhase : -1,
                ...
                // And not on a hull whose turret has been blown off - the live
                // turret, the wreck's drooped one and the gun are one part, and
                // the part is gone. See TankSprite.Turretless.
                Clocks.Body => Tank.ShowTurret && !Tank.Turretless ? 0 : -1,
                ...
                //
                // And the phase is composed, not passed through: the tube's
                // clock counts recoil, while the layer's phase axis carries
                // laying as well. Handing the recoil pose straight to a laid
                // layer is what walked the gun up and down the ladder on every
                // shot - see AtlasSet.RecoilPhases. The rung is the tank's own
                // now: it is laid at what it is shooting at, the board deciding
                // the angle - see TankSprite.BarrelRung and Gunnery.LayDeg. It
                // starts at level, which is the rung this used to be pinned to,
                // and stays where it was last laid once it has a target: a gun
                // is not lowered because the order was dropped, any more than
                // the ring is swung back - Main.UpdateAttack leaves both.
                Clocks.Recoil => Tank.ShowTurret && !Tank.Turretless
                    ? Tank.Atlas.BarrelPhase(Tank.BarrelRung, Tank.RecoilPhase)
                    : -1,
