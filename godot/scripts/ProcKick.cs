...
public sealed partial class ProcKick : Node3D
{
    ...
    /// <summary>
    /// What this cloud is made of, which is two different events sharing one
    /// model - see <see cref="Dress"/>.
    /// </summary>
    public enum Cloud
    {
        /// <summary>Propellant over the ground a gun is standing on: white,
        /// young, with the burning half still in it.</summary>
        Muzzle,

        /// <summary>Ground thrown up by something landing on it: earth, no fire,
        /// and none of the haze that makes a shot read as smoke.</summary>
        Ground,
    }

    /// <summary>
    /// Wear one or the other, writing every number either of them owns.
    ///
    /// <b>Both ways every time, because the pool hands the same cloud round</b> -
    /// <see cref="Might"/>'s finding, said about colour instead of size: a throw
    /// that only set what it wanted would inherit the rest from whoever used the
    /// slot last, and a gun would fire brown smoke after a tree fell. Skipped
    /// when the cloud is already wearing it, so a bench panel editing these is
    /// only overwritten when the kind actually changes hands.
    ///
    /// <b>The defaults are written here as well as in the shader text</b> -
    /// <see cref="ReachDefault"/>'s arrangement, and here it is unavoidable:
    /// these uniforms are declared at their working values, so there is nothing
    /// to read an "off" position out of. The two copies have to be kept in step.
    ///
    /// <b>The ground pair is the burst's earth, and the young end is lifted the
    /// way the pale pair was</b>: the shared light term shades a dense mass to
    /// about a quarter, so a dark stop at 0.115 comes out nearly black in the
    /// core - the note beside <c>pale_dark</c>, and it applies to any pair put
    /// here. And it does not walk white-to-grey, because raised earth does not:
    /// what a shot does between its second frame and its last is burn out, and
    /// what a crown coming down does is settle.
    /// </summary>
    public void Dress(Cloud kind)
    {
        if (_wearing == kind)
            return;
        _wearing = kind;
        bool ground = kind == Cloud.Ground;
        Tint("pale_dark", ground ? EarthYoungDark : PaleDark);
        Tint("pale_lit", ground ? EarthYoungLit : PaleLit);
        Tint("smoke_dark", ground ? EarthOldDark : SmokeDark);
        Tint("smoke_lit", ground ? EarthOldLit : SmokeLit);
        // The veil is what is still standing when the pale mass has gone, and it
        // is the family that reads as smoke: a gun leaves some hanging, a trunk
        // hitting dirt does not.
        Dial(Part.Dust, "veil_ink", ground ? GroundVeilInk : VeilInk);
        Dial(Part.Glow, "core_gain", ground ? 0.0f : CoreGain);
        Dial(Part.Glow, "ember_gain", ground ? 0.0f : EmberGain);
    }

    private Cloud _wearing = Cloud.Muzzle;

    private void Tint(string uniform, Vector3 to) =>
        _dustInk?.SetShaderParameter(uniform, to);

    /// <summary>The numbers <see cref="Dress"/> writes, in both positions. The
    /// muzzle column is the shader's own text and has to be kept in step with
    /// it; the ground column is this effect's second answer.</summary>
    public const float CoreGain = 0.58f;
    public const float EmberGain = 0.46f;
    public const float VeilInk = 0.62f;
    public const float GroundVeilInk = 0.26f;
    public static readonly Vector3 PaleDark = new(0.680f, 0.668f, 0.642f);
    public static readonly Vector3 PaleLit = new(0.995f, 0.988f, 0.962f);
    public static readonly Vector3 SmokeDark = new(0.560f, 0.562f, 0.578f);
    public static readonly Vector3 SmokeLit = new(0.985f, 0.986f, 0.996f);
    public static readonly Vector3 EarthYoungDark = new(0.500f, 0.435f, 0.335f);
    public static readonly Vector3 EarthYoungLit = new(0.940f, 0.870f, 0.715f);
    public static readonly Vector3 EarthOldDark = new(0.330f, 0.278f, 0.212f);
    public static readonly Vector3 EarthOldLit = new(0.735f, 0.640f, 0.485f);
    ...
    /// <summary>
    /// Which rung this cloud draws on - <see cref="Stage3D.DressOrder"/> or
    /// <see cref="Stage3D.StandOrder"/>.
    ///
    /// <b>A gun's dust stands, a ram's lies down</b>, and the difference is where
    /// the cloud is against the hull that made it. Muzzle dust is thrown out ahead
    /// of the shooter, clear of its own silhouette, so it belongs in the band that
    /// sorts by foot like everything else standing on the board. Two hulls meeting
    /// throw the ground out from <em>between</em> them, and a quad seated there
    /// covers whichever of the two is nearer the camera - a white veil laid across
    /// the armour, which is the trap <c>Stage3D.Mine</c> already names word for
    /// word: a cloud on the board is a quad in the world and a tank is a billboard
    /// in front of it. On the dress rung it is dust on the ground between two
    /// tanks, which is what it is.
    ///
    /// Written on every throw, because the clouds come out of a pool -
    /// <see cref="ProcSlam.Order"/>'s reason: one dropped a rung for a ram and
    /// handed back would put the next gun's dust under the tank that fired it.
    /// </summary>
    public int Order
    {
        get => _dustInk?.RenderPriority ?? _order;
        set
        {
            _order = value;
            if (_dustInk is not null)
                _dustInk.RenderPriority = value;
            if (_glowInk is not null)
                _glowInk.RenderPriority = value;
        }
    }

    private int _order = Stage3D.StandOrder;
