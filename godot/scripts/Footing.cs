...
{
	/// <summary>
	/// One hull's footprint on the ground: where it stands, its two ground axes
	/// as they land on screen, and how far it reaches along each.
	///
	/// <b>The pair of axes is the whole of it.</b> The board is drawn in
	/// isometric, so a rectangle on the ground is a parallelogram on screen and
	/// the two axes are not perpendicular there;
	/// <see cref="AtlasSet.GroundDirection"/> gives them as they land, and
	/// solving an offset in that pair takes the projection back out. That
	/// convention was written once, for a mine under a hull
	/// (<see cref="Mines.Under"/>), and this is the same question asked by
	/// anything else that wants to know whether a point on the board is under a
	/// tank - a felled trunk, for one. Kept here rather than copied there,
	/// because two Cramers keyed off one atlas is one Cramer that will be wrong.
	/// </summary>
	public readonly record struct Tread(Vector2 At, Vector2 Ahead,
										Vector2 Across, float Half, float Wide)
	{
		/// <summary>The footprint of one tank, or null if it has no atlas to
		/// take its axes from.</summary>
		public static Tread? Of(Vehicle tank, Vector2 origin = default)
		{
			if (tank.Atlas is not { } atlas)
				return null;
			double hull = tank.Sprite.HullFacing;
			float half = atlas.HullSpan * 0.5f * tank.Sprite.BodyScale;
			return new Tread(tank.GroundPoint - origin,
							 atlas.GroundDirection(hull),
							 atlas.GroundDirection(hull + 90.0),
							 half, half * Mines.Broad);
		}

		/// <summary>
		/// Whether a point on the ground is under this hull, and how far along
		/// the hull's own axis it lies (positive toward the glacis).
		///
		/// <paramref name="reach"/> stretches the box forward only, in hull
		/// half-lengths: the ground a tank is about to be standing on is
		/// answered by the same box as the ground it is standing on, and a
		/// stretch backwards would claim what it has already left.
		/// </summary>
		public bool Covers(Vector2 point, out double along, float reach = 0.0f)
		{
			along = 0.0;
			float det = Ahead.X * Across.Y - Ahead.Y * Across.X;
			if (Math.Abs(det) < 1e-6f)
				return false;
			Vector2 off = point - At;
			float a = (off.X * Across.Y - off.Y * Across.X) / det;
			float b = (Ahead.X * off.Y - Ahead.Y * off.X) / det;
			along = a;
			return a >= -Half && a <= Half * (1.0f + reach)
				   && Math.Abs(b) <= Wide;
		}
	}
