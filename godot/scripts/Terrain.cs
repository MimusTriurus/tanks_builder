...
public sealed class GroundRule
{
    ...
    /// ramp - deep water, and nothing else so far.
    ///
    /// <b>The way in and not the way out.</b> A bank is a step down into the
    /// pond, and a tank drives off it into the water the way a man walks off a
    /// jetty; the way back up is the same step and wants a ramp, which the
    /// ordinary level rule in <see cref="HexField.Passable"/> already says.
    /// Read there and nowhere else. GDD states.md "Плывёт", docs/swim-plan.md.
    /// </summary>
...
public static class TerrainRules
{
    ...
    private static readonly Dictionary<Cover, CoverRule> Overs = new()
    {
        ...
        {
            Burns = true,
            // <b>Cleared is the heavy's doing and nobody else's</b> - GDD
            // classes.md, "Бульдозер HT": a wood the heavy drove into becomes
            // ground. It is the minefield's word rather than a fourth one
            // because it is the minefield's fact - a cover that was used up and
            // is no longer there - and the state a wood arrives in does not
            // matter: alive or burnt out, the trees go down the same.
            States = new[]
            {
        ...
        {
            // <b>Not hidden, and the rules say so in as many words</b> - GDD
            // field.md: "мины видны обоим игрокам всегда... скрытой случайности
            // в них нет". Their job is to shape movement and to punish a tank
            // shoved on to one, and neither wants a secret. It stood true here
            // from the day the table was written and was read by nothing, which
            // is exactly how a flag becomes a rule nobody decided.
    ...
    /// <summary>Whether a tank may drop on to this floor from a level above
    /// with no ramp - see <see cref="GroundRule.Plunge"/>.</summary>
    public static bool Plunges(Foundation ground) => Of(ground).Plunge;
