...
public sealed class BodyPitch
{
    ...
    /// <summary>Whether the spring has anything left to do - angle or velocity.
    /// The tick runs a standing tank's spring only while this says so, and a
    /// jolt is all velocity on its first frame, so the angle alone would leave
    /// it parked.</summary>
    public bool Moving => Angle != 0.0 || _velocity != 0.0;

    /// <summary>Hit the body: an impulse on the spring, in angle per second.
    /// Positive drives the nose down. The turret coming down on the engine deck
    /// (<see cref="TurretToss"/>) is the one caller; at -1.8 the tail dips
    /// about 3px on a 68px hull and is still in a fifth of a second.</summary>
