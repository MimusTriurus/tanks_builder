...
public sealed class Vehicle
{
    ...
    /// <summary>
    /// The heading it is being shoved along, or null when it is driving itself.
    ///
    /// <b>A tank that is pushed keeps the hull heading it had</b> - GDD
    /// units.md, "цель отбрасывается на соседний гекс по направлению тарана,
    /// сохраняя своё направление корпуса" - and the order machinery has no other
    /// way of saying so: <c>TankTick.AdvanceOrder</c> swings the hull on to the
    /// heading of every leg it is given, because until this field there was no
    /// leg a tank had not chosen. It is the heading rather than a flag so that
    /// what is being said is on the vehicle: "this hull is going that way without
    /// facing it".
    ///
    /// Cleared where the shove ends - see <c>TankTick.Shove</c>. Not written by
    /// anything the player does: an order given to a tank that is being pushed
    /// replaces the push, and <c>CancelOrder</c> is the one place that says so.
    /// </summary>
    public int? Shoved;
    ...
    /// <summary>
    /// How far above its cell's face the hull rides, in screen px: nought
    /// everywhere but in deep water, where a hull with the wading gear floats -
    /// <see cref="TankTick.Buoyancy"/>, less what a drowning has taken
    /// (<see cref="TankTick.Afloat"/>).
    ///
    /// <b>Already inside <see cref="Height"/>, <see cref="Standing"/> and the
    /// sprite's position</b>; kept here so a frame can tell what the ride was
    /// and move by the difference rather than re-deriving the whole stand. The
    /// swimmer used to stand on the bed and have its waterline ruled to the
    /// deck; it stands on its water now, at <c>surface - draught</c>, and the
    /// waterline is measured as a ford's is - docs/swim-plan.md, step 2.
    /// </summary>
    public float Float;

    /// <summary>The bob on that ride - see <see cref="Buoy"/>. Kicked by the
    /// tick on the frame the hull drops into deep water, run down every frame,
    /// and inside <see cref="Float"/> whenever it is not nought.</summary>
    public readonly Buoy Bob = new();

    /// <summary>Whether the hull was in water on the previous frame - the tick's
    /// own memory, so the frame it goes in can be told from the frames it is in.
    /// That frame is the splash, the douse and the kick (docs/swim-plan.md,
    /// step 3); kept on the vehicle because the tick keeps nothing per vehicle
    /// of its own.</summary>
    ...
    /// <summary>How long a fire that has been put out takes to leave the
    /// screen, and how long the flame itself lasts of that.
    ///
    /// <b>Two clocks because putting a fire out is two things happening at
    /// different speeds.</b> The flame goes almost at once - that is what water
    /// does to it - and the steam it leaves hangs for about a second. One clock
    /// would have to choose, and either choice is wrong: a flame that lingers a
    /// second is a fire that was not put out, and steam that vanishes with the
    /// flame is the hard cut this replaced.
    /// </summary>
    public const double DouseSeconds = 1.15;

    /// <inheritdoc cref="DouseSeconds"/>
    public const double DouseFlameSeconds = 0.35;

    /// <summary>Seconds since the fire was put out. Starts spent, so a tank
    /// that has never burned is not quietly steaming on the first frame.
    /// </summary>
    private double _doused = DouseSeconds;

    /// <summary>
    /// Put the fire out - the rules' "потушен", with its picture.
    ///
    /// <b>A method rather than <c>Burning = false</c>, and the difference is
    /// the whole point.</b> The flag going false is what a reset does: this
    /// tank is not on fire any more and never mind how it stopped. An event
    /// putting a fire out is a thing that happens on screen, and the second it
    /// takes is this clock. Both spellings stay, and which one a caller wants
    /// is decided by whether anybody was meant to see it.
    /// </summary>
    public void Douse()
    {
        Burning = false;
    ...
    /// <summary>How much of the doused fire's smoke is still up: 1 on the frame
    /// it went out, 0 once it has gone. Nought for a tank that is burning or
    /// has never burned.</summary>
    public double Steam => SteamAt(_doused);

    /// <summary>The last of the flame under that steam, on its own shorter
    /// clock - see <see cref="DouseSeconds"/>.</summary>
    public double Ember => EmberAt(_doused);

    /// <summary>The two ramps as bare arithmetic, so they can be checked - a
    /// required-member type cannot be conjured in a test, which is
    /// <see cref="WadingAt"/>'s own reason for existing.</summary>
    public static double SteamAt(double since) =>
        1.0 - Math.Clamp(since / DouseSeconds, 0.0, 1.0);

    /// <inheritdoc cref="SteamAt"/>
    public static double EmberAt(double since) =>
        1.0 - Math.Clamp(since / DouseFlameSeconds, 0.0, 1.0);

    /// <summary>One frame of the fire going out. Runs whether or not anything
    /// is drawn, because a clock that only advances while its own picture is up
    /// cannot be the thing that takes the picture down.</summary>
    public void Dousing(double delta) => _doused += delta;
    ...
    ///
    /// <b>The tube follows the rung and the lane does not, and that is not an
    /// inconsistency.</b> The mouth is a place on a picture, so it moves when
    /// the gun is laid (<see cref="AtlasSet.Muzzle(double, int)"/>); <c>Along</c>
    /// is where the round goes on the board, and a board is flat. How far off
    /// the board the round rises between here and there is the shot's own
    /// business - <c>Shell.Apex</c> - and saying it twice is what put the round
    /// and the tube in different places to begin with.
    /// </summary>
    public (Vector2 Tube, Vector2 Along) Bore(double heading) =>
        (Atlas.Muzzle(Sprite.TurretFacing, Sprite.BarrelRung) - Atlas.Anchor,
    ...
    /// </summary>
    /// <summary>
    /// Where a round that came down from above struck this hull: over the ring
    /// axis, at the height the roof of the silhouette stands at, in the
    /// sprite's own frame.
    ///
    /// <b><see cref="Blown"/>'s point with no plate under it</b>, and that is
    /// the whole of what a mortar's bomb is on this board: the four plates are
    /// the hull's sides sorted by its heading, the roof is none of them, and
    /// the GDD says so in as many words - "для навесного огня миномёта зона
    /// значения не имеет". So there is no face to ask for a centroid, no
    /// tangent to scatter along and no slope to rise up; what is left is a
    /// point, and this is it.
    ///
    /// <b>The ring axis rather than the centre of the deck</b>, because the ring
    /// axis is the one point on a tank this project measures everything from -
    /// the sprite's own origin is it, and hull, turret and hex all turn about
    /// it. Sideways scatter would need the deck's own two half-extents, which
    /// the atlas does not stamp: there is no roof plate in the render, and
    /// asking for one is pipeline work for a burst whose own width is most of a
    /// hull.
    ///
    /// <see cref="AtlasSet.DeckHeightPx"/> and not the top of the height range:
    /// the range runs to a hatch or an engine cover, and what a bomb lands on is
    /// the plate the turret sits on.
    /// </summary>
    public Vector2 Roof() =>
        Atlas.GroundOffset - new Vector2(0.0f, (float)Atlas.DeckHeightPx);
    ...
    /// <summary>
    /// Which of the six axes a round arriving from <paramref name="from"/>
    /// leaves this hull along, or -1 for a plate that swallows it.
    ///
    /// <b><see cref="Graze"/>'s answer read as an axis instead of as a
    /// direction</b>, and it is here for <see cref="Graze"/>'s own reason: two
    /// callers want it - the round that flies on (<c>TankTick.Carry</c>) and
    /// the bench that has to know where to stand a tank to catch it
    /// (<c>Playback.Ricochet</c>) - and both would otherwise assemble the
    /// plate's outward bearing out of the same two atlas calls. The rule itself
    /// is <see cref="Gunnery.Deflect"/>, which knows nothing about this tank.
    ///
    /// The overload taking the plate is for a caller that already has one: a
    /// shell carries the face it hit, and asking the atlas for it again would
    /// be the same choice made twice off two different hull headings if the
    /// tank turned in between.
    /// </summary>
    public int Deflection(double from) =>
        Deflection(Atlas.FaceFor(from, Sprite.HullFacing), from);

    /// <inheritdoc cref="Deflection(double)"/>
    public int Deflection(string face, double from) =>
        Gunnery.Deflect(face, from, Sprite.HullFacing + Atlas.HitBearing(face));
    ...
    {
        // <b>A destroyed hull is not on its cell for anybody who asks this.</b>
        // GDD states.md, "Уничтожен": the hex no longer blocks movement or the
        // line of fire - so it is not a target, not a victim to ram, not a hull
        // to click, and a move on to it is a move on to open ground. The wreck is
        // still in the list, because the picture and the ash map still need it.
        foreach (Vehicle vehicle in vehicles)
            if (vehicle.Cell == cell && !vehicle.Wreck.Dead)
                return vehicle;
        return null;
    }
    ...
    public static int SelectionFor(IReadOnlyList<Vehicle> vehicles, int active,
                                  Vector2I cell)
    {
        // Not a destroyed one: its cell is open ground to a click, as it is to
        // a shell and a route - At's rule.
        for (int i = 0; i < vehicles.Count; i++)
            if (i != active && vehicles[i].Cell == cell && !vehicles[i].Wreck.Dead)
                return i;
        return -1;
    }
                                            ...
                                            Vehicle mover)
    {
        ...
        // The destroyed are not in the way - At's rule, and for its reason.
        foreach (Vehicle vehicle in vehicles)
            if (vehicle != mover && !vehicle.Wreck.Dead)
