...
public sealed partial class SelectionRing : Node2D
{
    ...
    /// <summary>
    /// The violet one, for a cell that has been picked rather than a tank.
    ///
    /// <b>Chosen against what is already on the ground, which is the whole of
    /// the choice.</b> The cyan and the red are taken; the route is drawn in
    /// yellow-green and its destination in amber (<c>HexField.RouteInk</c>,
    /// <c>DestInk</c>), and the firing arcs in a pale pink. Violet is the one
    /// hue on a board of earth, grass and water that none of those can be
    /// mistaken for, and it is the one hue the ground itself never is - so the
    /// mark reads as put there rather than as a patch of something.
    /// </summary>
    public static readonly Color Chosen = new(0.78f, 0.55f, 1.0f);
