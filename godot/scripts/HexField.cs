...
public sealed partial class HexField : Node2D
{
    ...
    public bool Passable(Vector2I cell, int heading)
    {
        ...
        if (!InBounds(cell) || !InBounds(next))
            return false;
        // The floor, before any of the height arithmetic. Rock is a refusal of
        // the same kind as the board's edge - nothing about the step across it
        // makes it drivable - and off-board cells come through here too, which
        // is where Plot finally reaches the pathing. Rock was already
        // unreachable by standing two levels up; saying it here as well costs
        // nothing and means a board that flattens its relief does not quietly
        // open its cliffs. Deep water used to be refused here too; it is driven
        // into now, and how - a drop off the bank - is the last line of this
        // method.
        ...
        // Level to level - or one level down on to a floor that takes a drop,
        // which is deep water (TerrainRules.Plunges): a tank drives off the bank
        // into the pond with no ramp, the way it falls in when rammed. The way in
        // only. From the pond the bank is a level up, a level up wants a ramp, and
        // that is the line this one falls through to; so "out by the ramp alone"
        // is not a second rule but the first one, read from the water.
        return LevelAt(next) == LevelAt(cell)
               || (TerrainRules.Plunges(FoundationAt(next))
                   && LevelAt(next) == LevelAt(cell) - 1);
    }

    /// <summary>
    /// Whether the step from one cell to the next is a fall rather than a drive:
    /// a level down with no ramp at either end.
    ///
    /// <b>Nothing may be ordered along such a step</b> - see
    /// <see cref="Passable"/>, which refuses it everywhere but into deep water.
    /// Two things take it anyway, and both are the board doing something to a
    /// tank rather than a tank driving: the drop off a bank into the pond, and a
    /// hull thrown off a bank by a ram (<see cref="Ramming"/>). They are one
    /// question, so they are one method: what the hull does on the way down is
    /// the same shape whether there is water at the bottom of it.
    /// </summary>
    public bool Drops(Vector2I from, Vector2I onto) =>
        RampHeading(from) < 0 && RampHeading(onto) < 0
        && LevelAt(onto) < LevelAt(from);
    ...
    /// <summary>
    /// How deep the deep water stands over its bed, as a fraction of one level.
    ///
    /// <b>Under one, and the rim is the point.</b> It used to be exactly one
    /// <see cref="Lift"/> - filled to the brim, the surface level with the bank -
    /// and the note on <see cref="WaterRiseAt"/> called a fraction near one a rim
    /// nobody drew. The rim is asked for now: a pond whose surface stands a little
    /// under the shore reads as a hole full of water rather than as a blue plate,
    /// and the ramp into it keeps a dry head to drive off. Nothing in the rules
    /// reads the number - they know deep from shallow and high water from low,
    /// not how far under the shore the surface is - so it is a picture setting,
    /// and the slider says so.
    ///
    /// A ramp into deep water is flooded over the back <see cref="DeepDepth"/> of
    /// its run, exactly as a ford's beach is over <see cref="WaterDepth"/> of its
    /// own. See docs/swim-plan.md, step 0.
    /// </summary>
    public double DeepDepth = 0.85;

    /// <summary>The deep water's depth in screen px.</summary>
    public float DeepRise => Lift * (float)DeepDepth;

    /// <summary>
    /// How far under the level it counts as the bed of deep water is <i>drawn</i>,
    /// as a fraction of one level.
    ///
    /// <b>A picture, not a level, and the difference is the whole point.</b> The
    /// rules read the surface: a swimming tank stands on its water's level, fires
    /// and is fired at there, and gets out by a one-level ramp. Nothing in them
    /// reads the bottom. A real second level under the pond would put a drowned
    /// hull behind the near bank (two levels of drop against half a hex of screen
    /// row hides two thirds of it) and want a two-level ramp the map guards
    /// refuse as a waterfall. So the level stays at -1, and only the drawn bed -
    /// the plate the water stands over, the flank of the pond at the board's cut,
    /// the walls of the bank - goes lower by this much. The water shader reads its
    /// thickness off the depth buffer, so a deeper bed darkens the water on its
    /// own. See docs/swim-plan.md, step 0.
    ///
    /// Flat cells only: a ramp into the pond keeps its shape, because its head
    /// has to meet the bank. Its foot then stands over the drawn bed, under the
    /// water, which is named there as the cost.
    ///
    /// <b>A whole level by default, and the number is the drowned's.</b> A hull
    /// floats at <c>surface - draught</c> (<see cref="TankTick.Buoyancy"/>) and
    /// may sit no lower than the drawn bed; with the bed at its level the events
    /// pond is 39px deep and a medium drawn at the default size wants 48 to sit
    /// where its class says, so half a level under was the swimmer's number.
    /// The drowned wants more: it goes down to the bed and has to end under the
    /// water entire, turret and all, and where the water is shallower than the
    /// tank is tall it is drawn smaller by the excess
    /// (<see cref="Stage3D.Shrunk"/>). The five roofs stand 67 to 85px over
    /// the contact at the default size (<see cref="AtlasSet.RoofRisePx"/>); a
    /// level under gives 86px of water over the bed and nothing shrinks, where
    /// half a level gave 62 and a medium ended at three quarters of itself,
    /// which read as a toy. docs/swim-plan.md, step 9.
    ///
    /// The price is twice the old one. The board's slab, which is as thick as
    /// its deepest hole, is a level thicker on any board with deep water on it.
    /// And a hull drowned in the pond's <i>near</i> row stands behind the bank
    /// to its turret, because the bank is a level and a half of wall against
    /// half a hex of screen row; a swimmer is not, it floats at the surface
    /// whatever the bed does. Judge the drowned in the far row, and turn this
    /// down (<c>--deep-bed</c>, the panels' rows) if the near row matters more
    /// than the size.
    /// </summary>
    public double DeepBed = 1.0;

    /// <summary>
    /// Where a cell's floor is drawn, in screen px off the datum: the level's own
    /// height everywhere but on flat deep water, whose bed is <see cref="DeepBed"/>
    /// lower. The one statement of it, read by the stage's mesh, the pond's flank
    /// and nothing that decides anything - <see cref="TopAt"/> is what the rules
    /// read, and the two part only where <see cref="DeepBed"/> says so.
    /// </summary>
    public float BedAt(Vector2I cell) =>
        LevelAt(cell) * Lift
        - (IsDeep(cell) && RampHeading(cell) < 0 ? Lift * (float)DeepBed : 0.0f);
    ...
    /// </summary>
    public float WaterTop(Vector2I cell) => LevelAt(cell) * Lift + WaterRiseAt(cell);

    /// <summary>
    /// Whether a cell's water is deep: the hollow filled nearly to the brim, the
    /// surface most of a level over the bed and a little under the bank
    /// (<see cref="DeepDepth"/>). Read off the floor -
    /// <see cref="Foundation.Deep"/> is the one statement of it, made by the map's
    /// <see cref="BoardMap.DeepWet"/> letter - so a flooded cell that named no
    /// floor is a ford.
    /// </summary>
    public bool IsDeep(Vector2I cell) => FoundationAt(cell) == Foundation.Deep;

    /// <summary>
    /// How far the surface stands over a cell's floor, in screen px.
    ///
    /// <b>Depth is per cell now, and this is where the two depths meet.</b> A
    /// ford stands <see cref="WaterRise"/> over its bed, which is one slider; deep
    /// water stands <see cref="DeepRise"/> over its own, which is the other - most
    /// of a <see cref="Lift"/>, a little under the bank rather than level with it,
    /// see <see cref="DeepDepth"/> for why. The waterfall guard keeps the two out
    /// of one body - see <see cref="MapRules.StepFaults"/>.
    /// </summary>
    public float WaterRiseAt(Vector2I cell) => IsDeep(cell) ? DeepRise : WaterRise;
    ...
    /// <summary>Whether either end of a leg is deep water - a swim, which
    /// <see cref="TankTick.SpeedCap"/> caps below a wade. The same shape as
    /// <see cref="IsWet"/>, for its reason: getting in and getting out both
    /// cost.</summary>
    public bool IsDeepLeg(Vector2I from, Vector2I onto) =>
        IsDeep(from) || IsDeep(onto);
    ...
    public List<Vector2I> FindPath(Vector2I from, Vector2I to,
                                   ...
                                   bool masonry = false, bool bulldozer = false)
    {
        ...
        while (queue.Count > 0 && !found)
        {
            ...
            foreach (int heading in EdgeHeadings)
            {
                ...
                if (!InBounds(next) || cameFrom.ContainsKey(next)
                    ...
                    // A heavy crosses it and breaks it, which is a step and not
                    // an obstacle - MovementProfile.Bulldozes, GDD classes.md.
                    || (masonry && !bulldozer && Blocked(cell, heading)))
                    continue;
                cameFrom[next] = cell;
                if (next == to)
                {
    ...
    /// <summary>
    /// Whether the wood on a cell has been driven flat - the heavy's doing, GDD
    /// classes.md "Бульдозер HT", docs/fell-plan.md.
    ///
    /// <b>Asked as the pair rather than as <see cref="HexFace.Standing"/>.</b>
    /// The general question is "is this cover still there", and on a board that
    /// authored no covers a hashed wood answers <see cref="Cover.None"/> - so
    /// <c>Standing</c> is false on every drawn wood that no map named, and a
    /// grove asking it would refuse to sow the trees it is drawing. This asks
    /// the one thing that is a fact: the cell says wood, and the wood is spent.
    /// </summary>
    public bool Felled(Vector2I cell) =>
        CoverAt(cell) == Cover.Forest
        && CoverStateAt(cell) == CoverState.Cleared;
    ...
    /// <summary>
    /// How many times what is on the cells has changed - <c>Craters.Stamp</c>'s
    /// twin and its argument word for word: a marker on a mine is a fact until the
    /// mine goes off, so redrawing it every frame would be work done to say nothing
    /// happened. Bumped by everything that writes a cover or one of their states -
    /// <see cref="SetCover"/>, <see cref="SetCoverState"/> and <see cref="Breach"/> -
    /// because the reader cannot tell which of them it was.
    /// </summary>
    public int CoverStamp { get; private set; }
    ...
    /// <see cref="Masonry.Headings"/> index - that is, <see cref="EdgeIndex"/>,
    /// counting from 330 and <b>not</b> the ascending <see cref="EdgeHeadings"/>
    /// - or null on a board with no walls.
    ///
