...
not a tank or a tank the pipeline cannot yet drive. Object names below are the
parts-built canon (`Hull.World`, `Turret.World`, pipeline/docs/tank-scene.md); an
old-style scene has `world` with `Hull` and `Turret` under it, read the summary.
...
- **Front axis.** `front_dir = 270` is a declared convention, not a measurement.
  Render one top-down ortho view and look at which end the headlights/glacis
  are on. Do not guess from the name or from the bounding box.
- **Stray base plate or unseated parts** are a generator-era problem: the
  recipes (`find_plate_cut`, `strip_plate`, seating by height map) live under
  `assemble_tank` in pipeline/docs/tools.md and are needed only if the summary shows a
  slab or a floating turret.
...
of 6; the tanks use **24** (15° per frame, and every layer of one tank must use
the same count - CLAUDE.md, "Конвенции"). Pass `hex_labels` with the measured `front_dir` so every frame carries a
...
spec = importlib.util.spec_from_file_location(
    "sprite_atlas", r"D:\Projects\AgentCoding\BlenderMCP\pipeline\sprite_atlas.py")
mod = importlib.util.module_from_spec(spec); spec.loader.exec_module(mod)
report = mod.render_set({
    "shared": {"output_dir": r"D:\Projects\AgentCoding\BlenderMCP\pipeline\out",
               "steps": 24, "tile": 256, "azimuth": 0.0, "elevation": 30.0,
               "spin_pivot_from": ["Turret"],
               "hex_labels": {"front_dir": 270.0, "orientation": "flat"}},
    "layers": [{"name": "hull", "target": "Hull.World"},
               {"name": "turret", "target": "Turret.World"},
